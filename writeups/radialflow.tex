...
Consider a perfect gas in which pressure $p$, density $\rho$, and sound
speed $a$ are related through a constant ratio of specific heats $\gamma$
according to
\begin{align}
    \gamma p &= \rho a^2.
\end{align}
...
assumed to be invariant flow to connect $a$ and $u$ with a reference specific
enthalpy $h_0 = \frac{\theta a_0^2}{\gamma_0-1}$ given fixed
$\theta\in\mathbb{R}^{+}$ and a reference velocity $u_0$.  That is,
\begin{align}
     H
     &=
        \frac{       a^2  }{\gamma  -1}       + \frac{1}{2} \vec{u}^2
      = \frac{\theta a_0^2}{\gamma_0-1} + \frac{1}{2} u_0^2
      =  H_0
...
\begin{align}
    \frac{p}{\rho} &< \frac{\theta a_0^2}{\gamma-1} + \frac{1}{2}u_0^2
\label{eq:constant_specific_stagnation_enthalpy_realizablity}
\end{align}
is necessary for realizability. After a rearrangement, constant
stagnation enthalpy condition~\eqref{eq:constant_specific_stagnation_enthalpy},
\begin{align}
        a^2
     &=   \frac{\gamma-1}{\gamma_0-1} \theta a_0^2
...
\begin{align}
       \vec{u}\cdot \vec{\nabla}\vec{u}^2
    &= \left[
          2 \frac{\gamma-1}{\gamma_0-1} \theta a_0^2 
...
\begin{align}
  a^2 &= \theta + \Mach[0]^2\frac{\gamma_0-1}{2}\left(1-\vec{u}^2\right)
\label{eq:stagnation_sound_nondim}
\end{align}
where clearly a realizable $a^2>0$ requires
\begin{align}
  u^2_\text{valid} &< \frac{2\theta}{\Mach[0]^2\left(\gamma_0-1\right)} + 1
.
...
\begin{align}
    ...
    \left(2\theta\Mach[0]^{-2} + \gamma_0 - 1\right)
    <
    u^2_\text{valid}
\label{eq:u_sonic}
.
...
\begin{align}
    \frac{p}{\rho} &< \frac{\theta}{\gamma_0-1} + \frac{\Mach[0]^2}{2}
...
\begin{align}
       \left(
           2 u^2
         - \left[
              2 \theta \Mach[0]{}^{-2} + \left(\gamma_0-1\right) \left(1 - u^2\right)
           \right]
       \right)
       u^\prime
    &=
...
condition on either radius $R_1$ or $R_2$.
