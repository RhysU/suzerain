...
and using linearity one arrives at Giles' result
\begin{align}
...
.
$$
The characteristic polynomial expressed as a function of $\lambda_y$ is
\begin{align}
-\left(\frac{-1+u {\lambda_x}+v {\lambda_y}+w {\lambda_z}}{v}\right)^4
\frac{1}{a^2-v^2} \biggl(
     u^2 {\lambda_x}^2
  &+ 2 u {\lambda_x} \left(-1+v {\lambda_y}+w {\lambda_z}\right)
\\
  &+ \left(-1+v {\lambda_y}+w {\lambda_z}\right)^2
   - a^2 \left({\lambda_x}^2+{\lambda_y}^2+{\lambda_z}^2\right)
\biggr)
.
\end{align}
