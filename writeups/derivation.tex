...
terms, and nondimensionalizes the results.
...
\subsection{The Favre-averaged Navier--Stokes equations}

In the absence of body forces and heat sources,
equations~\eqref{eq:cons_mass},\eqref{eq:cons_momentum},
...
&=
  ...
  - \nabla\cdot{}q
\end{align}
where we have abbreviated the surface heat flux $q_s$ to $q$ and employed total
enthalpy $H$ to reduce the number of terms in the energy equation.

A straightforward-but-lengthy sequence of algebra (detailed in section~2
...
&=
    \nabla\cdot\left(
        \left(\bar{\tau}
      - \overline{u''\otimes\rho{}u''}\right)
    \tilde{u}\right)
  - \nabla\cdot\left(
        \bar{q}
      + \overline{u''\otimes\rho{}h''}
    \right)
  + \nabla\cdot\left(
        -\frac{1}{2}\overline{\rho{}{u''}^{2}u''}
      + \overline{\tau{}u''}
    \right)
\end{align}
\end{subequations}
Several unclosed correlations impact the evolution of mean quantities: the
Reynolds stress $-\overline{u''\otimes\rho{}u''}$, the Reynolds heat flux
$\overline{u''\otimes\rho{}h''}$, turbulent transport
$-\frac{1}{2}\overline{\rho{}{u''}^{2}u''}$, and turbulent work
$\overline{\tau{}u''}$.  The Reynolds stress and heat flux augment the viscous
stress and heat flux, respectively.  The turbulent transport and work terms
represent transport of the turbulent kinetic energy density
$k=\frac{1}{2}\widetilde{{u''}^2}$ and viscous stress work due to turbulent
velocity fluctuations, respectively.

The turbulent kinetic energy density $k$ arises from averaging the total energy
given by \eqref{eq:perfectgastotalenergy}:
\begin{align}
  \rho{} E
&=
...
energy, is defined by $\bar{\rho} \epsilon = \overline{\tau : \nabla{}u''}$.
Some of the  pressure terms will soon be reinterpreted in light of our
constitutive relations.

We need to average the constitutive relations chosen in
section~\ref{sec:constitutive}.  Taking the Reynolds average
of~\eqref{eq:perfectgaseos} gives
\begin{align}
  \bar{p} &= R\overline{\rho{}T} = \bar{\rho}R\tilde{T}
\end{align}
while the Favre average of~\eqref{eq:perfectgasenthalpy} is
\begin{align}
 \tilde{h} &= \frac{\gamma{}R\tilde{T}}{\gamma-1}.
\end{align}
Following page~216 of Lele~\cite{Lele1994Compressibility}, expanding $h$,
averaging, removing the mean state from both sides, and applying perfect gas
assumptions shows the exact relationship
\begin{align}
  \overline{u''}
&=
...
.
\end{align}
Employing $h''$ in place of $T''$ and differentiating one obtains
\begin{align}
  \overline{p'u''}
&=
  \bar{p} \frac{\widetilde{h''u''}}{\tilde{h}}
- \bar{p} \overline{u''}
\\
  \nabla\cdot \overline{p'u''}
&=
  \nabla\cdot \bar{p} \frac{\widetilde{h''u''}}{\tilde{h}}
...
- \nabla\cdot\bar{p} \frac{\widetilde{h''u''}}{\tilde{h}}
\end{align}
allowing us exactly exchange the pressure velocity correlation
$\overline{p'u''}$ for the Reynolds heat flux within the $k$ equation:
\begin{align}
\label{eq:fanstke}
    \frac{\partial{}}{\partial{}t}\bar{\rho}k
  + \nabla\cdot(\tilde{u}\otimes\bar{\rho}k)
...
\\
 &+ \bar{p}\nabla\cdot\overline{u''}
  ...
  + \overline{p' \nabla\cdot{}u''}
\end{align}
That exchange reduces by one the number of correlations appearing in the $k$
equation which do not appear in the mean continuity, momentum, or energy
equations.

Using the strain rate tensor $s=\frac{1}{2}\left( \nabla{}u+\trans{\nabla{}u}
\right)$ and its deviatoric component $S=s-\frac{1}{3}\trace{s}\,I$ we
reexpress~\eqref{eq:taunewt} and~\eqref{eq:secondviscosityclaw} as
\begin{align}
  \tau
&= \mu \left( \nabla{}u+\trans{\nabla{}u} \right)
...
\begin{align}
   \bar{\tau}
&= 2 \bar{\mu}\tilde{S} + 2 \overline{\rho\nu''S''}
  + \alpha \bar{\mu} \left(\nabla\cdot\tilde{u}\right) I
  + \alpha \overline{\rho\nu''\nabla\cdot{}u''} I
.
\end{align}
Some FANS closure approximations neglect correlations between the kinematic
viscosity and velocity derivatives.  Many assume $\alpha=0$.  Those
approximations eliminate the second through fourth terms in $\bar{\tau}$.

To find $\bar{q}$ we combine~\eqref{eq:fourierlaw} and our assumption of a
...
\subsubsection{Contributions to the mean viscous stress tensor}

\subsubsection{Contributions to the mean heat flux tensor}



\subsection{Nondimensional equations using Suzerain's constitutive assumptions}

\subsection{Sampling statistical quantities from a simulation}

% TODO Ergodicity and homogeneity allowing us to pick zero-zero modes
%      Use channel_treatment.tex for some of the necessary language
