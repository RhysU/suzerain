...
in the preceding discussion.  In the context of
...
\\
% \underbrace{
   &- \frac{\gamma-1}{2} \reference{m^{2}\rho^{-2}}\nabla\rho
    + \left(\gamma-1\right)\trans{\nabla{}m}\reference{\rho^{-1}m}
    - \frac{\gamma-1}{\Mach^2}\nabla{}e
% }_{-\Mach^{-1}\nabla{}p}
\\
% \underbrace{
   &- \Reynolds^{-1} \reference{\mu\rho^{-2}m} \Delta\rho
    - \Reynolds^{-1} \nabla\nabla\rho \reference{\left(\mu+\lambda\right)\rho^{-2}m}
    + \Reynolds^{-1} \reference{\mu\rho^{-1}} \Delta{}m
    + \Reynolds^{-1} \reference{\left(\mu+\lambda\right)\rho^{-1}} \nabla\nabla\cdot{}m
% }_{\Reynolds^{-1}\nabla\cdot\tau}
\\
...
\\
  &+ \frac{\gamma}{\Reynolds\Prandtl\left(\gamma-1\right)}
     \reference{ \mu\rho^{-2}\left(
         \left(\gamma-1\right)\Mach^{2} \frac{m^2}{2\rho} - p
     \right) } \Delta\rho
   - \frac{\gamma\Mach^{2}}{\Reynolds\Prandtl}
     \reference{\mu\rho^{-2}m}\cdot\Delta{}m
   + \frac{\gamma}{\Reynolds\Prandtl}\reference{\mu\rho^{-1}}\Delta{}e
\end{align}
where the pointwise reference values are abbreviated as
\begin{align}
  c^{m}_{\nabla\rho} &= \reference{m^{2}\rho^{-2}}
&
  \overrightarrow{c^{m}_{\nabla{}m}}
  &= \reference{\rho^{-1}m}
&
  \overrightarrow{c^{m}_{\Delta\rho}}
  &= \reference{\mu\rho^{-2}m}
\\
  \overrightarrow{c^{m}_{\nabla\nabla\rho}}
  &= \reference{\left(\mu+\lambda\right)\rho^{-2}m}
&
  c^{m}_{\Delta{}m} &= \reference{\mu\rho^{-1}}
&
  c^{m}_{\nabla\nabla\cdot{}m}
  &= \reference{\left(\mu+\lambda\right)\rho^{-1}}
\\
...
\\
  c^{e}_{\Delta\rho} &=
&
  \overrightarrow{c^{e}_{\Delta{}m}}
  &=
&
  c^{e}_{\Delta{}e} &=
\end{align}
Vector references values are built from three scalar components.  For
example,
\begin{align}
  \overrightarrow{c^{m}_{\nabla{}m}} &= \trans{\left(
      c^{m_{x}}_{\nabla{}m},
...
from~\eqref{sec:formingoperators}, we need a discrete implementation of
$M+\varphi{}L$ for arbitrary $\varphi$, $k_m$, and $k_n$.  Switching to a
blocked matrix representation employing five scalar conserved state fields, the
...
\end{bmatrix}
\end{align}
While written above using blocked submatrices, in the implementation the state
may be interleaved such that the overall discrete operator bandwidth is five
times the bandwidth of the B-spline mass matrix $M$.
