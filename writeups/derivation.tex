...
% Environment sidewaystable from rotating plays poorly with amsart class
...
\begin{align*}
\renewcommand{\arraystretch}{5.0} % Adds whitespace between rows
\begin{bmatrix}
% Density row
  \entry{\rho\rho}
% M
...
\end{bmatrix}
\begin{bmatrix}
  \rho \left(\km, \cdot, \kn\right)\\
  \mx  \left(\km, \cdot, \kn\right)\\
  \my  \left(\km, \cdot, \kn\right)\\
  \mz  \left(\km, \cdot, \kn\right)\\
  e    \left(\km, \cdot, \kn\right)\\
\end{bmatrix}
\end{align*}
where blocked vectors written using curly braces are dotted against the blocked
vector of submatrices $\trans{\begin{bmatrix} \M & \D{1} & \D{2}
\end{bmatrix}}$
\caption[The discrete operator $M+\varphi{}L$ used for implicit time advance]{
 ...
 time-and-substep dependent operator is a function of $\varphi$, $k_m$, $k_n$,
 and diagonal reference quantity matrices, e.g. $C^\nu$.
}
