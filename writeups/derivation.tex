...
analysis to three directions with $y$ being the inhomogeneous direction (see
Equation 4.71).  Guarini proved that the linear structure of the Euler
...
state variable and nondimensionalization choices.  For complete details
Guarini's thesis and the papers of Giles should be consulted.

For the state vector $U = \left\{ \rho, u, v, w, p, \rho\phi \right\}$ the
dimensional Euler equations, augmented by a conserved passive scalar equation,
...
&
 C &= \begin{bmatrix}
       ...
       \end{bmatrix}
\end{align}
The conserved passive scalar $\phi$ has been added in anticipation of future
needs.  Any number of conserved passive scalars will behave in a manner
identical to the final row and column of matrices $A$, $B$, and $C$.  Consider
perturbations $\delta{}U = \left\{ \delta{}\rho, \delta{}u, \delta{}v,
\delta{}w, \delta{}p, \delta{}\rho\phi \right\}$ taken about some steady,
uniform reference state $U_0$ so that $U = U_0 + \delta{}U$.  These
perturbations evolve according to the linearized Euler equations
$$
...
= 0
$$
where matrices $A_0$, $B_0$, and $C_0$ are evaluated at $U_0$.
