...
$y$ being the inhomogeneous direction.

We review Giles' approach following Guarini.  For complete details, especially
motivations and proofs, the papers of Giles and Guarini's thesis should be
consulted.  For the state vector $U = \left\{ \rho, u, v, w, p, \rho\phi
\right\}$ the dimensional Euler equations, augmented by a conserved passive
scalar equation, may be written as follows:
$$
...
&
 C &= \begin{bmatrix}
       ...
       \end{bmatrix}
\end{align}
The conserved passive scalar $\phi$ has been added to this derivation in
anticipation of future simulation needs.  Any number of scalars will behave
identically to the final row and column of matrices $A$, $B$, and $C$.

Consider perturbations $\delta{}U = \left\{ \delta{}\rho, \delta{}u, \delta{}v,
\delta{}w, \delta{}p, \delta{}\rho\phi \right\}$ taken about some steady,
uniform reference state $U_0$ so that $U = U_0 + \delta{}U$.  The short-time
evolution of perturbations is governed by the linearized Euler equations
\begin{align}
...
\end{align}
holds.
Applying $- \ii^{-1} B_0^{-1}$ to equation~\eqref{eq:dimeulerreduced}, one
finds a family of eigenvalue problems in $k_y$
$$
  \left( \omega B_0^{-1} - k_x B_0^{-1} A_0 - k_y I - k_z B_0^{-1} C_0 \right)
  \delta\hat{U}^R = 0
$$
where $\delta\hat{U}^R$ is the eigenvector and a solution to the right null
space problem.  The left null space problem
$$
  V^{L}
  \left( \omega B_0^{-1} - k_x B_0^{-1} A_0 - k_y I - k_z B_0^{-1} C_0 \right)
  = 0
...
following Giles, used the orthogonality properties of eigenvectors satisfying
the dispersion to build the exact, nonreflecting boundary conditions
\begin{align}
\label{eq:dimeulerexact}
  V_n^L \delta{}U &= 0
\end{align}
for each $V_n^L = V^L\!\left(k_{y_n}\right)$ corresponding to an incoming wave.
This exact condition is approximated for reasons of computational tractability.
Dividing the dispersion relation~\eqref{eq:dimeulerdisp} by $\omega$ and
noticing $V_n^L$ therefore is only a bivariate function of $\lambda_x =
k_x/\omega$ and $\lambda_z/\omega$ suggests employing a Taylor series.
Truncating the series is equivalent to assuming waves have a small angle of
incidence.  To first order,
\begin{align}
...
and using linearity one arrives at the result
\begin{align}
...
Comparing the above result with equation~\eqref{eq:spatial_discretization} and
the implicit operator from section~\ref{sec:discretizationofimplicitterms}, the
condition fits well into Suzerain's implicit discretization.  The implicit
operator rows corresponding to nonreflecting boundaries must modified at every
wavenumber.  Care must be taken to only adjust the incoming or outgoing
characteristics.  Two issues, however, must be addressed.
...
$$
may be used to rewrite the approximate boundary
condition~\eqref{eq:dimeulerapprox} as
\begin{align}
