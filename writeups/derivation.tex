...
\usepackage{fancyhdr}
\usepackage{fullpage}
\usepackage[final,hypertexnames=false]{hyperref}
\usepackage{ifthen}
\usepackage{lastpage}
\usepackage{latexsym}
\usepackage{mathrsfs}
...
\\
  \frac{\partial}{\partial{}t} e = \dots
   &- \overrightarrow{c^{e}_{\nabla\rho}} \cdot\vec{\nabla}\rho
    ...
    + \dots
\end{align}
where the repeated reference values are
\begin{align}
...
&
  ...
  = \trans{\left(c^{\nu{}u}, c^{\nu{}v}, c^{\nu{}w}\right)}
\end{align}
and the single-use reference values are
\begin{align}
...
\\
  ...
  \right) }
\end{align}
with superscripts indicate the relevant equation and
subscripts indicating the related term.
...
$M+\varphi{}L$ for arbitrary $\varphi$, $k_m$, and $k_n$.  To accommodate
wall-normal variation in reference quantities, notice diagonal matrices defined
like
\begin{align}
  C^{\nu} &= \begin{bmatrix}
   \left.c^{\nu}\right|_{y=0} & 0     & 0 \\
...
state fields, the complete, SMR91-ready implicit operator is:
\begin{align}
\begin{bmatrix}
%
  \rho{}\rho  &
