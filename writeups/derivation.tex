...
complex to implement.  Guarini~\cite{Guarini1998} summarizes an extension of
Giles' results to three spatial dimensions without reproducing the necessary
analysis.  Medida~\cite{Medida2007} lucidly catalogs the intermediate results
necessary in three dimensions.

We now review Giles' approach following Guarini's notation with the goal of
arriving at Medida's results.  For complete details, especially motivations and
proofs, the work of Giles, Medida, and Guarini should be consulted in that
respective order.  For the state vector
$$
...
U = U_0 + \delta{}U.
$$
The short-time evolution of perturbations is governed by the linearized Euler
equations
\begin{align}
...
the magnitude of the reference velocities relative to the sound speed $a$, are
required to determine how many characteristics are entering or exiting through
the boundary.  The left null space problem
\begin{align}
...
following Engquist and Majda, used the orthogonality properties of eigenvectors
to build the exact, nonreflecting boundary conditions
\begin{align}
...
Multiplying through by $-\ii\omega$, Fourier transforming in both time and
space, and using the assumption that $U_0$ is both steady and uniform yields
\begin{align}
...
\end{align}
where the cumbersome $\lambda_y,\lambda_z=0$ notation is herein and henceforth
suppressed.  Inserting ${V_n^L}^{-1} V_n^L$ twice,
\begin{align}
  \frac{\partial}{\partial{}t}
  V_n^L
  \delta{}U
...
.
\end{align}
Defining characteristic variables using the action of $V_n^L$, \textit{viz.}
\begin{align}
  \delta{}C &= V_n^L \delta{}U
  ,
\end{align}
allows rewriting equation~\eqref{eq:dimeulerapprox} in a compact
form showing temporal interactions amongst the various waves,
\begin{align}
...
permits translating these results to another set of state variables $V$ with
