...
\\
  ...
  \right)}
\\
  c^{e}_{\nabla\cdot{}m} &= \reference{\rho^{-1}\left(
        \gamma{}e - \frac{\gamma-1}{2}\mbox{Ma}^{2} \rho^{-1}m^2
  \right)}
\\
  c^{e}_{\Delta\rho} &= \reference{ \mu\rho^{-2}\left(
         \left(\gamma-1\right)\Mach^{2} \frac{m^2}{2\rho} - p
  \right) }
\end{align}
...
from~\eqref{sec:formingoperators}, we must implement $M+\varphi{}L$ for
arbitrary $\varphi$, $k_m$, and $k_n$. $L$ is nothing but the discrete form of
the linear terms from the previous section.  To accommodate wall-normal
variation in reference quantities, notice diagonal matrices defined like
\begin{align}
  C^{\nu} &= \begin{bmatrix}
   \left.c^{\nu}\right|_{y=0} & 0     & 0 \\
   0                         & \ddots & 0  \\
   ...
   \end{bmatrix}
\end{align}
will scale the application of operators like $C^{\nu}D^{(2)}$ by
$\left.c^{\nu}\right|_{y=y_l}$ at collocation point $y=y_l$.

Switching to a blocked matrix representation employing five scalar conserved
state fields, the complete, SMR91-ready discrete operator is shown in
table~\vref{tbl:discreteimplicitop}.  Though written using submatrices
blocked assuming each conserved scalar is stored contiguously, in the
implementation the five conserved scalars may be interleaved such that the
overall operator bandwidth is five times that of the B-spline mass matrix $M$.

\begin{sidewaystable}
\centering
\vspace{2in}                      % Move table inside page margins
\renewcommand{\arraystretch}{2.0} % Adds whitespace between rows
\newcommand{\entry}[1]{}          % Provides comments for subblocks
\newcommand{\C}[2]{C^{#1}_{#2}}   % For brevity below
\newcommand{\D}[1]{D^{(#1)}}      % For brevity below
\newcommand{\M}{M}                % For brevity below
\newcommand{\g}{\gamma}           % For brevity below
\newcommand{\km}{k_{m}}           % For brevity below
\newcommand{\kn}{k_{n}}           % For brevity below
\newcommand{\Ma}{\Mach}           % For brevity below
\newcommand{\mx}{m_{x}}           % For brevity below
\newcommand{\my}{m_{y}}           % For brevity below
\newcommand{\mz}{m_{z}}           % For brevity below
\newcommand{\vp}{\varphi}         % For brevity below
\begin{align*}
\begin{bmatrix}
% Density row
  \entry{\rho\rho}
...
\\\entry{\mx\rho }
  -\frac{\g-1}{2}\ii\km\C{\mx}{\nabla\rho}\M
& \entry{\mx\mx  }
% M
  -\left(\g-1\right)\ii\km\C{u}{}\M
& \entry{\mx\my  }
  -\left(\g-1\right)\ii\km\C{v}{}\M
& \entry{\mx\mz  }
  -\left(\g-1\right)\ii\km\C{w}{}\M
& \entry{\mx{}e  }
% Wall-normal momentum row
\\\entry{\my\rho }
  -\frac{\g-1}{2}\C{\my}{\nabla\rho}\D{1}
& \entry{\my\mx  }
  \left(\g-1\right)\C{u}{}\D{1}
& \entry{\my\my  }
% M
  \left(\g-1\right)\C{v}{}\D{1}
& \entry{\my\mz  }
  \left(\g-1\right)\C{w}{}\D{1}
& \entry{\my{}e  }
% Spanwise momentum row
\\\entry{\mz\rho}
  -\frac{\g-1}{2}\ii\kn\C{\mz}{\nabla\rho}\M
& \entry{\mz\mx }
  -\left(\g-1\right)\ii\kn\C{u}{}\M
& \entry{\mz\my }
  -\left(\g-1\right)\ii\kn\C{v}{}\M
& \entry{\mz\mz }
% M
  -\left(\g-1\right)\ii\kn\C{w}{}\M
& \entry{\mz{}e }
% Total energy row
\\\entry{e\rho  }
& \entry{e\mx   }
...
\end{bmatrix}
\begin{bmatrix}
  \rho \left(\km, y_l, \kn\right)\\
  \mx  \left(\km, y_l, \kn\right)\\
  \my  \left(\km, y_l, \kn\right)\\
  \mz  \left(\km, y_l, \kn\right)\\
  e    \left(\km, y_l, \kn\right)\\
\end{bmatrix}
\end{align*}
\caption[The discrete operator $M+\varphi{}L$ used for implicit time advance]{
 The discrete operator $M+\varphi{}L$ used for implicit time advance.  The
 operator is a function of $\varphi$, $k_m$, $k_n$, and diagonal reference
 quantity matrices, e.g. $C^\nu$.
}
\label{tbl:discreteimplicitop}
\end{sidewaystable}
