...
The convective criterion uses the maximum eigenvalue of the Euler equations as
a surrogate for the more complicated Navier--Stokes case.  Both
Kwok~\cite{Kwok2002} (equation~2.39) and Guarini~\cite{Guarini1998}
...
For formulations like Suzerains in which an explicit Mach number
...
nondimensionalization.  Using a hybrid implicit/explicit time scheme with
acoustic terms computed implicitly effectively sets the sound speed to be zero
when computing the convective criterion.
...
periodic wall-normal basis.  In section~3.2 of his thesis, Venugopal presents a
linearized analysis taking into account the inhomogeneous nature of the
wall-normal direction.  He determined that the wall-normal imaginary eigenvalue
magnitude dropped by nearly an order of magnitude after taking into account the
...
$1/\Delta{}y$ \emph{four} times smaller than the nominal value was feasible
(equation~3.29).
