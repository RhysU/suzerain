...
\\
  &{}- \nabla\nabla\rho \lessreference{\left(\mu+\lambda\right)\rho^{-3}m^2}
\\
  &{}+ \reference{\left(\mu+\lambda\right)\rho^{-2}m}\cdot\nabla\nabla\cdot{}m
     - \nabla\nabla\rho \reference{\left(\mu+\lambda\right)\rho^{-3}m^2}
\end{align}
...
into the stability criteria set forth in~\ref{sec:stabilitycriteria}.  During
several hybrid implicit/explicit $\Mach=1/2$ channel flow simulations we
observed stability issues arising from the total energy equation while
treating the latter two terms explicitly.  We choose to linearize all four
terms to unify the numerics across the momentum and energy equations and for
the iota of additional stability linearizing the latter two provides.
...
allows using a time step safety factor closer to one without sacrificing
temporal accuracy.  This reduces the wall time necessary to obtain converged
statistics or allows finer resolution atop fixed computing resources.
...
\begin{align}
  ...
  \end{pmatrix}
\end{align}
while the other reference values
\begin{align}
  c^{m}_{\nabla\rho} &= \reference{m^{2}\rho^{-2}}
  = \trace\overleftrightarrow{c^{u\otimes{}u}}
\\
  \overrightarrow{c^{e}_{\nabla\rho}} &= \reference{
...
\begin{align*}
\bm{\vp}
\renewcommand{\arraystretch}{7.0} % Adds whitespace between rows
...
& \entry{\mx\mz  }
  \subcoeff{
    ...
    - \ii\kn\C{u_x}{}
    - \frac{\left(\alpha+\frac{1}{3}\right)}{\Re}\km\kn\C{\nu}{}
  }{
  }{
  }
& \entry{\mx{}e  }
...
N_y$ blocked vectors written using curly braces are to be ``dotted'' against
the blocked vector $
