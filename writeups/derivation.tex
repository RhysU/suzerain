...
acoustic waves generated by the flow cannot be leave the domain and will pile
up in the simulation.  A non-physical partition of the energy results and
spoils the utility of any statistics gathered.
...
Euler equations using primitive state variables.  Giles' boundary conditions
are selected over Poinsot and Lele's ``locally one-dimensional inviscid''
relations~\cite{Poinsot1992Boundary} because other codes with identical
discretization choices have successfully employed Giles' approach to our
problems of interest.  Rowley and Colonius~\cite{Rowley2000Discretely} present
higher order techniques expected to perform better than Giles' approach but
these suffer from considerably greater implementation complexity.
Guarini~\cite{Guarini1998} extended Giles' analysis to three directions with
$y$ being the inhomogeneous direction.  Medida~\cite{Medida2007} lucidly
catalogs the intermediate results necessary for the three dimensional
extension.  We review Giles' approach following Guarini.  For complete details,
especially motivations and proofs, the work of Giles, Guarini, and
Medida should be consulted.

For the state vector $U = \left\{ \rho, u, v, w, p, \phi \right\}$ the
dimensional Euler equations, using the ideal equation of state $\rho a^2 =
\gamma p$ and augmented by a passive scalar convection equation, may be written
as follows:
$$
...
&
 C &= \begin{bmatrix}
        ...
        0 & w & 0 & 0        & 0              & 0 \\
        0 & 0 & w & 0        & 0              & 0 \\
        0 & 0 & 0 & w        & \frac{1}{\rho} & 0 \\
        0 & 0 & 0 & \rho a^2 & w              & 0 \\
        0 & 0 & 0 & 0        & 0              & w
       \end{bmatrix}
\end{align}
The passive scalar $\phi$ has been added to this system in anticipation of
future simulation needs.  Any number of scalars will behave identically to the
final row and column of matrices $A$, $B$, and $C$.  When no scalar is
required, the associated entries may be omitted from an implementation.  Now
consider perturbations $\delta{}U = \left\{ \delta{}\rho, \delta{}u, \delta{}v,
\delta{}w, \delta{}p, \delta\phi \right\}$ taken about some steady, uniform
reference state $U_0$ so that $U = U_0 + \delta{}U$.  The short-time evolution
of perturbations is governed by the linearized Euler equations
\begin{align}
...
&= 0
\end{align}
where matrices $A_0$, $B_0$, and $C_0$ are evaluated at $U_0$.  Assuming
...
\label{eq:dimeulerdisp}
  ...
  .
\end{align}
Assuming $B_0$ is invertible and applying $- \left(\ii\omega{}B_0\right)^{-1}$
...
space problem.  The signs of the associated eigenvalues, in conjunction with
the magnitude of velocities $u$, $v$, and $w$ relative to the sound speed $a$,
are required to determine how many characteristics are entering or exiting
through the boundary.  The left null space problem
\begin{align}
\label{eq:dimeulereigenprob}
  ...
  \right) &= 0
\end{align}
naturally gives rise to the associated left null vector $V^L$.  Guarini,
following Giles, used the orthogonality properties of eigenvectors to build the
exact, nonreflecting boundary conditions
\begin{align}
\label{eq:dimeulerexact}
  V_n^L \delta{}U &= 0
\end{align}
for each $V_n^L = V^L\!\left(k_{y_n}\right)$ corresponding to \textit{either
incoming or outgoing waves}.  This exact condition is approximated using a
Taylor series in $\lambda_x$ and $\lambda_z$ for reasons of computational
...
\begin{align}
  \left.V_n^L\right|_{\lambda_x,\lambda_z=0}
  \delta{}U
  +
  \lambda_x
  \left.\frac{dV_n^L}{d\lambda_x}\right|_{\lambda_x,\lambda_z=0}
  \delta{}U
  +
  \lambda_z
  \left.\frac{dV_n^L}{d\lambda_z}\right|_{\lambda_x,\lambda_z=0}
  \delta{}U
  &\approx 0
  .
\end{align}
Multiplying through by
$-\ii\omega$ and Fourier transforming in time gives the
spatially-spectral result
\begin{align}
\label{eq:dimeulerapprox}
  ...
  .
\end{align}
Fourier transforming in space recovers the corresponding pointwise
expression,
\begin{align}
  \left.V_n^L\right|_{\lambda_x,\lambda_z=0}
  \frac{\partial}{\partial{}t}\delta{}U
  &\approx
  \left.\frac{dV_n^L}{d\lambda_x}\right|_{\lambda_x,\lambda_z=0}
  \frac{\partial}{\partial{}x}\delta{}U
  +
  \left.\frac{dV_n^L}{d\lambda_z}\right|_{\lambda_x,\lambda_z=0}
  \frac{\partial}{\partial{}z}\delta{}U
  .
\end{align}
As noted by Engquist and Majda~\cite{Engquist1977Absorbing} and later by
Trefethen and Halpern~\cite{Trefethen1986Wellposedness}, only some series
truncations of this form lead to well-posed conditions.  Moreover, the
straightfoward application of even this first order approximation requires
either ad~hoc~\cite{Giles1988Nonreflecting,Medida2007} or
systematic~\cite{Rowley2000Discretely} modification to produce well-behaved
inflow constraints.

Any implementation of the boundary conditions~\eqref{eq:dimeulerapprox} requires
the eigenvalues and left eigenvectors from
problem~\eqref{eq:dimeulereigenprob}.  The matrix of interest, \textit{viz}.
$B_0^{-1}-\lambda_x B_0^{-1}A_0 - \lambda_z B_0^{-1}C_0$, may be expanded as
$$
\begin{bmatrix}
 \frac{1-u {\lambda_x}-w {\lambda_z}}{v} & \frac{v {\lambda_x} \rho }{a^2-v^2} & \frac{\rho(1-u {\lambda_x}-w {\lambda_z})}{a^2-v^2} & \frac{v {\lambda_z} \rho }{a^2-v^2} & \frac{-1+u {\lambda_x}+w {\lambda_z}}{v \left(a^2-v^2\right)} & 0 \\
 0 & \frac{1-u {\lambda_x}-w {\lambda_z}}{v} & 0 & 0 & -\frac{{\lambda_x}}{v \rho } & 0 \\
 0 & -\frac{a^2 {\lambda_x}}{a^2-v^2} & \frac{v (-1+u {\lambda_x}+w {\lambda_z})}{a^2-v^2} & -\frac{a^2 {\lambda_z}}{a^2-v^2} & \frac{1-u {\lambda_x}-w {\lambda_z}}{\rho (a^2 -v^2) } & 0 \\
 0 & 0 & 0 & \frac{1-u {\lambda_x}-w {\lambda_z}}{v} & -\frac{{\lambda_z}}{v \rho } & 0 \\
 0 & \frac{a^2 v {\lambda_x} \rho }{a^2-v^2} & \frac{a^2 (1-u {\lambda_x}-w {\lambda_z}) \rho }{a^2-v^2} & \frac{a^2 v {\lambda_z} \rho }{a^2-v^2} & \frac{v (-1+u {\lambda_x}+w {\lambda_z})}{a^2-v^2} & 0 \\
 0 & 0 & 0 & 0 & 0 & \frac{1-u {\lambda_x}-w {\lambda_z}}{v}
\end{bmatrix}
.
$$
The characteristic polynomial expressed as a function of an unadorned $\lambda$ is
\begin{multline}
\left(
  \lambda^2
+ \lambda
  \frac{2 v \left(u {\lambda_x}+w {\lambda_z}-1\right)}{v^2-a^2}
+ \frac{u^2 {\lambda_x}^2+2 u {\lambda_x} \left(w {\lambda_z}-1\right)+\left(w {\lambda_z}-1\right)^2 - a^2 \left({\lambda_x}^2+{\lambda_z}^2\right)}{v^2-a^2}
\right)
\times{}{}
\\
{}\times{}
\left(\frac{u {\lambda_x}+v {\lambda}+w {\lambda_z}-1}{v}\right)^4
=0
.
\end{multline}
Two of the roots evidently come from a quadratic subproblem with discriminant
FIXME.

The characteristic polynomial zero of multiplicity four yields the four remaining eigenvalues
\begin{align}
\lambda_{3,4,5,6}
=
\frac{1-u {\lambda_x}-w {\lambda_z}}{v}
=
\frac{ \omega - u k_x - w k_z }{ \omega{} v }
.
\end{align}



The associated matrices have been calculated using commodity symbolic
manipulation software.  The results are:
FIXME
...
First, the result is not expressed for Suzerain's conservative state variables.
In section 4.3.2 of his thesis, Guarini~\cite{Guarini1998} proved that the
linear structure of the Euler equations permits translating Giles' approach to
another collection of state variables using the Jacobian matrix of the
coordinate transformation.  For some other state variable selection $V$,
reference state $V_0$, and perturbation $\delta{}V = V - V_0$, the Jacobian
matrix
$$
   S = \frac{\partial{}U}{\partial{}V}
$$
Guarini rewrote the approximate boundary condition~\eqref{eq:dimeulerapprox} as
\begin{align}
\label{eq:dimeulertransform}
  \left.V_n^L\right|_{\lambda_x,\lambda_z=0}
  S
  \frac{\partial}{\partial{}t}\delta{}V
  &\approx
  \ii k_x
  \left.\frac{dV_n^L}{d\lambda_x}\right|_{\lambda_x,\lambda_z=0}
  S
