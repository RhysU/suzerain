...
forming it within a contiguous region of memory.  ``Inverting'' the operator
(more correctly, factorizing and solving for unknown coefficients given
collocation point values) will.  Employing an appropriate permutation matrix
$P$ causes the renumbered operator $P\left(M+\varphi{}L\right)\trans{P}$ to be
globally banded with a small bandwidth independent of the number of wall-normal
degrees of freedom.  This $P\left(M+\varphi{}L\right)\trans{P}$ allows
efficient banded factorization and back substitution.
...
figure~\ref{fig:discreteimplicitop}.  Previous work by Guarini did not treat
density implicitly~\cite{Guarini1998}.  Finally, implicitly handling only the
