...
&
  ...
  \end{array}\right]
\end{align}
Direct computation shows $\det V^L = -2\bar{\rho}^3\bar{a}^5$ and so $V^L$ is
always nonsingular for a realizable reference state.  Medida's equations~(5.82)
and~(5.83) provide one pair of inflow and outflow conditions for which
reflection coefficients were not reported:
\begin{align}
...
The particular coordinate transformation Jacobian we require maps the primitive
state $U = \left\{ \rho, u, v, w, p \right\}$ to the conserved variables $V =
\left\{ \rho, \,\rho{}u, \,\rho{}v,\, \,\rho{}w, \,\rho{}E, \right\}$.
Observing that
\begin{align}
  u &= \frac{\rho{}u}{\rho}
&
...
&
  p &= \frac{\rho{}a^2}{\gamma}
     ...
     + \frac{1-\gamma}{2\rho}\left(
       ...
       \right)
\end{align}
permits computing the Jacobian evaluated at $\bar{U}$,
\begin{align}
...
.
\end{align}
Forming the product of the left eigenvectors and the Jacobian,
\begin{align}
V^L S &=
\left[\begin{array}{ccccc}
  \frac{1-\gamma}{\gamma}\bar{a}^2 & \bar{u}\left(1-\gamma-\frac{\bar{a}^2}{\bar{u}^2}\right) & \bar{v}\left(1-\gamma-\frac{\bar{a}^2}{\bar{v}^2}\right) & \bar{w}\left(1-\gamma-\frac{\bar{a}^2}{\bar{w}^2}\right) & \gamma-1-\frac{\bar{a}^2}{E} \\ \noalign{\smallskip}
  0                          & 0                                      & \bar{a}                                      & 0                                      & 0                      \\ \noalign{\smallskip}
  0                          & 0                                      & 0                                      & \bar{a}                                      & 0                      \\ \noalign{\smallskip}
  \frac{\bar{a}^2}{\gamma }        & \bar{u}\left(1-\gamma\right)+\bar{a}               & \bar{v}\left(1-\gamma\right)                 & \bar{w}\left(1-\gamma\right)                 & \gamma-1               \\ \noalign{\smallskip}
  \frac{\bar{a}^2}{\gamma }        & \bar{u}\left(1-\gamma\right)-\bar{a}               & \bar{v}\left(1-\gamma\right)                 & \bar{w}\left(1-\gamma\right)                 & \gamma-1               \\ \noalign{\smallskip}
\end{array}\right]
,
\end{align}
we find that
\begin{align}
  \det V^L S &= 2 \bar{a}^5 \left(1-\gamma+\frac{\bar{a}^2}{E \gamma } \right)
\end{align}
which can be zero only when
$$
  \frac{\bar{a}^2}{\gamma\left(\gamma-1\right)}
 = E
 = \frac{\bar{a}^2}{\gamma\left(\gamma-1\right)} + \frac{\bar{u}^2+\bar{v}^2+\bar{w}^2}{2}
$$
holds.  Therefore, our $V^L S$ is singular whenever $\bar{u}=\bar{v}=\bar{w}=0$.
