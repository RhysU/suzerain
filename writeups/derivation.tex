...
Combining the B-spline and Fourier details, for scalar-valued $u$ one finds
\begin{align}
...
collocation point values but streamwise and spanwise coefficients.
Similarly, for a vector with components $u$, $v$, and $w$ one finds
\begin{align}
  \left.\nabla\cdot\begin{pmatrix}u \\ v \\ w\end{pmatrix}\right|_{m n}
&=
  \left.\left(\ii k_m M u + D^{(1)} v + \ii k_n M w\right)\right|_{m n}
\\
  \left.\nabla\begin{pmatrix}u \\ v \\ w\end{pmatrix}\right|_{m n}
&=
  \left.\begin{pmatrix}
...
\\
  \left.\Delta\begin{pmatrix}u \\ v \\ w\end{pmatrix}\right|_{m n}
&=
  \left.\begin{pmatrix}
...
&=
  ...
  \end{pmatrix}\right|_{m n}
\\
  \left.\nabla\times\nabla\times\begin{pmatrix}u \\ v \\ w\end{pmatrix}\right|_{m n}
&=
STARTHERE!
  \left.\begin{pmatrix}
    - k_m^2 M u       + \ii k_m D^{(1)} v + \ii \left(k_m + k_n\right) M w \\
...
&
  ...
  \Delta\frac{m}{\rho}
\end{align}
By employing the Helmholtz decomposition and smoothness assumptions, one may
show $\nabla\times\nabla\times\frac{m}{\rho}$ acts in an antidiffusive manner
on the solenoidal portion of the velocity field.
