...
$y$ being the inhomogeneous direction (Equation 4.71).

We review Giles' approach following Guarini.  For complete details the papers
of Giles and Guarini's thesis should be consulted.  For the state vector $U =
\left\{ \rho, u, v, w, p, \rho\phi \right\}$ the dimensional Euler equations,
augmented by a conserved passive scalar equation, may be written as follows:
$$
...
anticipation of future simulation needs.  Any number of conserved passive
scalars will behave in a manner identical to the final row and column of
matrices $A$, $B$, and $C$.
...
evolution of small perturbations is governed by the linearized Euler equations
\begin{align}
\label{eq:dimeulerperturb}
      \frac{\partial\delta{}U}{\partial{}t}
+ A_0 \frac{\partial\delta{}U}{\partial{}x}
+ B_0 \frac{\partial\delta{}U}{\partial{}y}
+ C_0 \frac{\partial\delta{}U}{\partial{}z}
= 0
\end{align}
...
$$ holds.
Applying $- \ii^{-1} B_0^{-1}$, one finds a family of wavenumber-dependent
eigenvalue problems for $k_y$
$$
...
the dispersion to arrive at the exact, nonreflecting boundary conditions
$$
  V_n^L \delta{}U = 0
$$
...
Guarini~\cite{Guarini1998} proved that the linear structure of the Euler
equations permits translating Giles' approach to another collection of state
variables using the Jacobian matrix of the coordinate transformation (see
Equation 4.81).  Guarini also rotated the nonreflecting boundary to match the
...
system is unnecessary (though Victor has employed modest grid stretching near
the freestream and other ``solution conditioning'' tools like low pass
filtering).

We now translate Guarini's orthogonal, dimensional results to Suzerain's
state variable and nondimensionalization choices.
