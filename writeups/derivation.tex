...
table~\vref{fig:discreteimplicitop}.  Though written using submatrices blocked
assuming each conserved scalar is stored contiguously, in the implementation
the five conserved scalars may be interleaved such that the overall operator
bandwidth is only five times that of the B-spline mass matrix $M$.
...
depicted.  The $3 N_y \times N_y$ blocked vectors written using curly braces
are dotted against the blocked vector $
  \trans{\begin{bmatrix} \M & \D{1} & \D{2} \end{bmatrix}}
   $.
Each of $M$, $\D{1}$, and $\D{2}$ are $N_y \times N_y$ banded matrices.
Reference quantities like $C^\nu$ are $N_y \times N_y$ diagonal matrices.
The operator, a function of $k_m$ and $k_n$, takes wall-normal B-spline
coefficients to B-spline collocation point values.
