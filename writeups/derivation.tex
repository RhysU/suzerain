...
\begin{align}
  ...
  H &= E + \frac{p}{\rho}
     = C_{p} T + \frac{u\cdot{}u}{2}
     = \frac{\gamma{}RT}{\gamma-1} + \frac{u\cdot{}u}{2}
  &
  h &= H - \frac{u\cdot{}u}{2}
     = C_{p} T
...
along with the relationships
  ...
  p &= \left(\gamma-1\right) \left(
    \rho{}E - \Mach^{2} \rho{}\frac{u\cdot{}u}{2}
  \right)
  \\
...
Finally, wherever necessary, realizations of random fields of flow quantities---e.g.,
$u_i$, $p$, $\tau_{ij}$, etc.---are assumed to be differentiable in
both time and space.  Thus, the average and differentiation operators
commute.  For example,
%
\begin{equation*}
...
\subsection{Equations for a single-species in local thermodynamic equilibrium}

In the absence of body forces and heat sources,
equations~\eqref{eq:cons_mass},\eqref{eq:cons_momentum},
and~\eqref{eq:cons_energy} may be written
\begin{align}
  \frac{\partial}{\partial{}t}\rho+\nabla\cdot\rho{}u &= 0
\\
  \frac{\partial{}}{\partial{}t}\rho{}u + \nabla\cdot(u\otimes{}\rho{}u)
