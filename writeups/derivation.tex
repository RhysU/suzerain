...
While the derivation up to result~\eqref{eq:dimeulertransformcharnot} has
assumed $\bar{U}$ and $\bar{V}$ are known and steady, in practice they are not.
For example, the instantaneous wall-normal outflow reference velocity generated
by a homogenized boundary layer problem is unknown.  While the mean value might
possibly be computed \textit{a priori}, such a calculation would only be
correct in a statistical sense.  It would certainly be wrong while simulating
transients prior to reaching stationarity. FIXME
