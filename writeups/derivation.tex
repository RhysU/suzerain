...
respectively.  Only the conditions arising from one set of submatrices is meant
to be applied.  For some $U_0$ possessing an agreed upon relationship between
...
should be the case for some problems computed with Suzerain.
...
reference state $V_0$ and therefore perturbations
$$
  \delta{}V = V - V_0.
$$
As Guarini alludes, because of their frame indifference the Euler equations
accommodate simple rotations and reflections.  In this new setting, the
coordinate transformation Jacobian matrix
$$
   S = \frac{\partial{}U}{\partial{}V}
$$
and any rotation matrix $R$ may be used to rewrite the exact nonreflecting
...
\label{eq:dimeulertransform}
  ...
  .
\end{align}
Choosing an $R\ne{}I$ permits orienting the nonreflecting condition to best
handle waves traveling normal to something other than the $x$ boundary.  Using
notation and matrices from the compact
representation~\eqref{eq:dimeulerapproxBG},
\begin{align}
...
spatially-homogenized boundary layer (i.e. $R\ne{}I$).  Personal communications
with Victor Topalian regarding his temporally-homogenized work suggests
reorienting the nonreflecting coordinate system is unnecessary.  However,
modest grid stretching near the freestream and other ``solution conditioning''
tools like low pass filtering have been employed in his work.  This choice,
too, might need to be reviewed.
