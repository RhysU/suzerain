...
meant to be applied.  For some $U_0$ possessing an agreed upon relationship
between $u_0$ and $a_0$, specifying $V^L$, ${V^L}^{-1}$, and these partitioned
...
assumed $U_0$ and $V_0$ are known and steady, in practice they are not.  For
example, the instantaneous wall-normal outflow reference velocity generated by
a homogenized boundary layer problem is unknown.  While the mean value might
