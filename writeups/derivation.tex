...
\label{eq:dimeulertransform}
  ...
  .
\end{align}
Applying $\left(V^L S\right)^{-1}$, and using notation from the compact
representation~\eqref{eq:dimeulerapproxBG} gives the result,
\begin{align}
...
The particular coordinate transformation Jacobian we require maps the
dimensional $U = \left\{ \rho, u, v, w, p \right\}$ to the nondimensional $V =
\left\{ \rho^{*},\,\rho^{*}u^{*}, \,\rho^{*}v^{*}, \,\rho^{*}w^{*},
\,\rho^{*}E^{*}, \right\}$ described by sections~\ref{sec:nondimrefq}
and~\ref{nondim_equations}.  Several intermediate results are pertinent:
\begin{align}
  \rho &= \rho_0 \rho^{*}
&
  u &= \frac{u_0 \rho^{*}u^{*}}{\rho^{*}}
&
  v &= \frac{u_0 \rho^{*}v^{*}}{\rho^{*}}
&
  w &= \frac{u_0 \rho^{*}w^{*}}{\rho^{*}}
\end{align}
\begin{align}
  p &= \frac{\rho_0 a_0^2  \rho^{*} {a^{*}}^2}{\gamma}
     = \rho_0 a_0^2 \left(\gamma-1\right)\rho^{*}E^{*}
     + \frac{\rho_0 u_0^2}{2\rho^{*}}\left(1-\gamma\right)\left(
           \left(\rho^{*}u^{*}\right)^2
         + \left(\rho^{*}v^{*}\right)^2
         + \left(\rho^{*}w^{*}\right)^2
       \right)
\end{align}
The Jacobian for this transformation evaluated at either a dimensionless or a
dimensional reference $\bar{U}$ is
\begin{align}
S = \frac{\partial{}U}{\partial{}V} &= \left[\begin{array}{ccccc}
    \rho_0                     % 11
  & \frac{\rho_0}{\bar{u}^{*}} % 12
  & \frac{\rho_0}{\bar{v}^{*}} % 13
  & \frac{\rho_0}{\bar{w}^{*}} % 14
  ...
  & 0                          % 45
  \\ \noalign{\medskip}
    \frac{\rho_0 a_0^2 {\bar{a}^{*}}^2}{\gamma}      % 51
  & \rho_0 u_0^2 \left(1 - \gamma\right) \bar{u}^{*} % 52
  & \rho_0 u_0^2 \left(1 - \gamma\right) \bar{v}^{*} % 53
  & \rho_0 u_0^2 \left(1 - \gamma\right) \bar{w}^{*} % 54
  & \rho_0 a_0^2 \left(\gamma-1\right)               % 55
  \\ \noalign{\medskip}
\end{array}\right]
\\
&= \left[\begin{array}{ccccc}
    \rho_0                     % 11
  & \frac{\rho_0 u_0}{\bar{u}} % 12
  & \frac{\rho_0 u_0}{\bar{v}} % 13
  & \frac{\rho_0 u_0}{\bar{w}} % 14
  & \frac{\rho_0 u_0}{\bar{E}} % 15
  \\ \noalign{\medskip}
    0                             % 21
  & \frac{\rho_0 u_0}{\bar{\rho}} % 22
  & 0                             % 23
  & 0                             % 24
  & 0                             % 25
  \\ \noalign{\medskip}
  ...
  & 0                             % 42
  & 0                             % 43
  & \frac{\rho_0 u_0}{\bar{\rho}} % 44
  & 0                             % 45
  \\ \noalign{\medskip}
    \frac{\rho_0 {\bar{a}}^2}{\gamma}          % 51
  & \rho_0 u_0 \left(1 - \gamma\right) \bar{u} % 52
  & \rho_0 u_0 \left(1 - \gamma\right) \bar{v} % 53
  & \rho_0 u_0 \left(1 - \gamma\right) \bar{w} % 54
  & \rho_0 a_0^2 \left(\gamma-1\right)         % 55
  \\ \noalign{\medskip}
\end{array}\right]
.
