...
numerics have successfully employed Giles' conditions to our problems of
interest.  Rowley and Colonius~\cite{Rowley2000Discretely} present higher order
techniques expected to perform better than Giles' approach but these suffer
from considerably greater implementation complexity.
Guarini~\cite{Guarini1998} reports an extension to three spatial dimensions
without reproducing the necessary analysis.  Medida~\cite{Medida2007} lucidly
...
especially motivations and proofs, the work of Giles, Guarini, and Medida
should be consulted.
...
Multiplying through by $-\ii\omega$ and Fourier transforming in time gives the
spectral result
\begin{align}
\label{eq:dimeulerapprox}
  ...
  .
\end{align}
% Fourier transforming in space recovers the corresponding pointwise expression,
% \begin{align}
%   \left.V_n^L\right|_{\lambda_x,\lambda_z=0}
%   \frac{\partial}{\partial{}t}\delta{}U
%   &\approx
...
%   .
% \end{align}
Guarini~\cite{Guarini1998} proved the linear structure of the Euler equations
permits translating this result to another set of state variables $V$ with
reference state $V_0$ and therefore perturbations $\delta{}V = V - V_0$.  In
this new setting, the coordinate transformation Jacobian matrix
$$
...
Moreover, the
straightforward application of even this first order approximation requires
