...
% http://www.gnu.org/software/octave/doc/interpreter/Nonlinear-Programming.html.
  ...
  x    =zeros(11, 1); l    =zeros(size(x0)); u    =zeros(size(x0));
  ...
  % Unpack the parameters into a returned struct.  On success, s.info == 101.
  s = struct();
  [s.x,s.obj,s.info,s.iter,s.nf] = sqp(x,baseflow_phi,[],baseflow_h,l,u);
  [s.dp_e,s.dstar,s.gam0,s.Ma,s.Ma_e,s.p1,s.R0,s.R1,s.rho1,s.T_e,s.u1] = s.x;
