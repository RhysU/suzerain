...
function [r, u, rho, p, a2, up, pp] = nozzle1(Ma0, gam0, R1, R2, u1, rho1, p1, ...
                                              reltol=sqrt(eps), abstol=sqrt(eps))

  [Ma02, gam0m1] = deal(Ma0**2, gam0 - 1);
  assert(u1**2 < 2 / Ma02 / gam0m1 + 1,
         'Ma0=%g, gam0=%g, u1=%g imply a**2 <= 0', Ma0, gam0, u1);
  up    = @(r,u) -(u./r) .* (2 + Ma02*gam0m1 - Ma02*(gam0m1  )*u.**2) ...
  ...
  a2    = 1 + 0.5*Ma02*gam0m1*(1 - u.**2);                 % Full accuracy
  up    = up(r, u);                                        % Shadow prior 'up'
  rho   = rho1 * exp(-Ma02 * cumtrapz(r, r.*u.*up ./ a2)); % Lower accuracy
  pp    = - Ma02 * rho.*u.*up;                             % Lower accuracy
...
%! nozzle1(Ma0, gam0, Router, Rinner, -2/7, 1, 1);  % Breaks if u1 = -1/3
...
%! pkg load odepkg; Ma0 = 1; gam0 = 1.4; Rinner = 1; Router = 2;
%! nozzle1(Ma0, gam0, Rinner, Router, -1/Ma0+sqrt(eps), 1, 1);
