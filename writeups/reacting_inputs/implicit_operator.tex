...
implicit solves, we will persue a formulation that treats only
...
an $N_y (N_s + 4) \times N_y (N_s + 4)$, we require that the
...
\S\ref{sec:complete_implicit_operator} these contributions are added
...
%
\begin{equation*}
-\lambda_{\exp} \left( v - \lambda_{\mathrm{exp}} \right)^5.
\end{equation*}
%
...
0 & -wv & 0 & w & 0 & 0 \\
0 & -v \left(E + \pp{p}{\rho} - \frac{p}{\rho}\right) & v \pp{p}{\rho u} & E + \frac{p}{\rho} + v \pp{p}{\rho v}  & v \pp{p}{\rho w} & v \pp{p}{\rho E}
\end{array} \right].
\end{equation*}
%
...
\subsection{The Complete Operator}
