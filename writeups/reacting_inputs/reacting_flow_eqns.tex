...
Specifically, Suzerain utilizes the Antioch library~\cite{?}.  For
further discussion of these models, see~\cite{?}.
...
with a Blottner curve fit~\cite{?} and the mixture viscosity is
computed from these species viscosities using Wilke's mixing
rule~\cite{?}.  As with the chemical reactions, the viscosity is
computed by Antioch~\cite{}.
...
rule~\cite{}.  Like the mixture viscosity, these capabilities are
provided by Antioch~\cite{}.
