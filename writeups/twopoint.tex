...
computation of $R_uv$ is most effectively performed in Fourier space via
...
$\hat{v}_{lmn}$ to convert them to $\hat{u}_{y_j m n}$ and $\hat{v}_{y_j m n}$.
...
\\
  &=
     ...
     \hat{v}_{y_j m n}
\end{align}
This Fourier coefficient product and summation may be computed directly in
Suzerain as the $z$ direction does not employ conjugate symmetry.
...
\end{align}
where the final expression equivalently uses only coefficients
available in a conjugate symmetric treatment of the $x$ direction.
