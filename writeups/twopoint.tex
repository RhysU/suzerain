...
\subsection{The two-point correlation
            as a function of separation in both $x$ and $z$}

For any two real-valued, continuous functions $u$ and $v$, by definition
\begin{align}
...
R_{uv} \left( r_x, y_j, r_z \right)
   = \frac{1}{L_x L_z}
     ...
     \,\mathrm{d}x
\end{multline}
As real-valued $v$ is necessarily conjugate symmetric, $\hat{v}^\ast =
\hat{v}$.  Rearranging,
\begin{multline}
R_{uv} \left( r_x, y_j, r_z \right)
   = \frac{1}{L_x L_z}
...
coefficient representations of $u$ and $v$.  Computation of $R_uv$ is most
effectively performed in Fourier space via conveniently parallel,
complex-valued wavenumber products once a collocation-based B-spline mass
matrix has been applied to $\hat{u}_{lmn}$ and $\hat{v}_{lmn}$ to convert them
to $\hat{u}_{y_j m n}$ and $\hat{v}_{y_j m n}$.

\subsection{The two-point correlation
            as a function of separation in only $x$}

Repeat derivation for $r_z = 0$.  Beware any conjugate symmetry gotchas
from half-complex storage.

\subsection{The two-point correlation
            as a function of separation in only $z$}

Repeat derivation for $r_x = 0$.  Beware any conjugate symmetry gotchas
from half-complex storage.
