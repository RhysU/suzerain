...
\lstinputlisting[language=Octave,
                 ...
                 radial flow solver implementation\label{lst:octave_nozzle}}]
                {notebooks/nozzle.m}
...
= - \operatorname{sgn} \left[ p_{e,\xi} \left(\Mach[e]^2 - 1\right) \right]
  ...
  .
\end{equation}
Sample logic combining these constraints appears in
...
\begin{figure}[p]
  \lstinputlisting[language=Octave,
                   ...
                   layer edge quantities\label{lst:octave_nozzle_match}}]
                  {notebooks/nozzle_match.m}
  \caption*{\centering\parbox{0.9\textwidth}{
    Given $\delta$, $\gamma_0$, $\Mach[e]$, and $p_{e,\xi}$
    a matching base flow is produced as follows:
    \begin{enumerate}
...
$\rho\!\left(R\right)=1$ were held constant.  The CEV-based inputs and computed
conditions at $R=\sqrt{R_0^2+\delta^2}$ are tabulated below.
