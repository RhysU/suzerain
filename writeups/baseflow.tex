...
direction defined to be positive for both sub- and supersonic problems.
The sound speed squared at $y=\delta$ and $y=0$ is given by
\begin{align}
  a^2_e
  &\equiv
...
nothing but temperature $T_e$ and $T_w$ because
...
parametric inputs appears in Listing~\ref{lst:octave_nozzle_qoi}.  The kernel
...
\lstinputlisting[language=Octave,columns=fixed,float,
                 caption={\texttt{nozzle\_qoi.m}: A kernel computing nozzle
                 quantities of interest\label{lst:octave_nozzle_qoi}},
                 basicstyle={\footnotesize\sffamily},frame=single]
