...
\begin{lstlisting}[language=Mathematica,columns=flexible,
                   basicstyle={\scriptsize\sffamily},frame=single]
...
$-a_0 < u\!\left(R_{2}\right) u_0 < 0$ which implies $-\Mach^{-1} <
u\!\left(R_{2}\right) < 0$.  With this condition, $-u$ increases and $p$
decreases when traversing the domain from~$R_{2}$ to~$R_{1}$.  However, the
problem becomes stiff as the flow approaches sonic.  Instead, specifying at
most a nearly sonic outflow via
\begin{align}
    -\left(1 + \epsilon \right) u_0 u\!\left(1\right) < -a_0
    &\implies
    -u\!\left(1\right) < -\Mach^{-1} \left(1 + \epsilon\right)^{-1}
\label{eq:cpfradial_nozzle_subsonic_bc}
\end{align}
for small $\epsilon$ is done in conjunction with taking $R_{2}$ large enough to
obtain the desired $u\!\left(R_{2}\right)$ inflow condition.
