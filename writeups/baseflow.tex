...
hand, computing local $\rho$ and $\vec{\nabla}p$ is often of interest.
...
Given some $u$ one also wants to be able to compute local thermodynamics state.
...
.
\end{align}
Finally~\eqref{eq:p_nondim} simplifies to
\begin{align}
  p'
  &=
  \Mach^2 \rho{} u u'
...
.
\end{align}
Notice~\eqref{eq:cpfradial_nondim_ode} easily supplies local $u'$ for the
