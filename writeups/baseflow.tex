...
be collected from some constant $y$ line.  Curve fits are created from that
...
\citet[\textsection{}97]{Landau2004Fluid}, the simpler case when the area
...
solve such problems.  Sample solutions based on code in
Listing~\ref{lst:octave_nozzle} appear in Figure~\ref{fig:sample_solns}.
Problem stiffness can be mitigated by computing the analytic Jacobian $u''$
from~\eqref{eq:cpfradial_nondim_ode} and integrating with the
\texttt{LSODE}\citep{Hindmarsh1980LSODE} functionality available within
\texttt{Octave}.
