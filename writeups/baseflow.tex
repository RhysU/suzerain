...
setting.  Such a translation permits using radial nozzle solutions within the
...
u_x$, etc. at $\left(-x,y\right)$ to take advantage of the symmetry present the
second quadrant relative to the first one.
...
\lstinputlisting[language=Octave,%float=tbp,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Producing nozzle flows to match boundary layer edge conditions}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

We wish to produce Cartesian base flows possessing
\begin{enumerate}
    \item a unit edge streamwise velocity $u_\xi$ magnitude
          regardless of sub- or supersonic conditions,
    \item some prescribed edge Mach number $\Mach[e]{}$, and
    \item some prescribed edge pressure gradient $p_{e,\xi}$.
\end{enumerate}
%
Considering first the unit edge velocity requirement,
\begin{equation}
    1
...
\end{equation}
%
Folding this constraint into the definition of the edge Mach number,
\begin{equation}
    \Mach[e]{}
    =
    \frac{\Mach[0]{} R_0}{R}
...
\end{align}
%
Turning to the pressure gradient,
\begin{align}
  p_{e,\xi}
  &=
...
\end{align}
for its positive root to obtain a suitable $R_0$.

One knows $\operatorname{sgn} u\!\left(R\right)$ \emph{a priori} in the
quadratic equation for $R_0$. To obtain a favorable pressure gradient when
$\Mach[e]<1$, choose $u\!\left(R\right)=-1$ so that  $\operatorname{sgn}
u\!\left(R\right) = -1$. When $\Mach[e]<1$, select the positive sign. That is,
\begin{align}
  0
  &= \begin{cases}
    R_0^2
    \left(1 - \Mach[e]^2\right) p_{e,\xi}
  + R_0
    \delta
  - \Mach[e]^2 \delta^2 p_{e,\xi}
  &\mbox{when } \Mach[e]<1, p_{e,\xi}<0
  \\
    R_0^2
  ...
  \end{cases}
\end{align}
gives $\frac{\mathrm{d}p}{\mathrm{d}r} < 0$. The converse may be taken to
achieve an adverse pressure gradient.
...
%%%implemented here.  Though a realizability constraint is implemented, in
%%%practice it seems not to be encountered by the chosen optimizer with this
%%%particular initialization procedure.  The second \texttt{nonlin\_residmin}
%%%invocation permits many iterations as pre-asymptotic convergence has been quite
%%%slow on one test (not shown).  Generally, fewer than 50 total iterations and a
%%%few core-seconds of compute time are required for convergence to
%%%$\sqrt{\epsilon}$-like sum-of-squared mismatch residuals.  After optimization
%%%completes, the full base flow profile may be computed by supplying the results
%%%along with $R_2 = \sqrt{R_0^2 + L_y^2}$ to Listing~\ref{lst:octave_nozzle}.
%%%
...
$\rho\!\left(R_1\right) = 1$ was fixed and $p\!\left(R_1\right)$ was set to
