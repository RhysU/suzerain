...
\begin{align}
  ...
  =
  - \frac{\Mach^2}{2}\int_{\Omega} \frac{\vec{\nabla}\vec{u}^2}{a^2} \, \mathrm{d}V
...
\begin{align}
  \oint_{\partial\Omega} p \, \mathrm{d}S
  &=
  \int_{\Omega} a^2 \vec{\nabla}\rho \, \mathrm{d}V
\label{eq:p_nondim}
\end{align}
...
\end{align}
%
Equation~\eqref{eq:stagnation_sound_nondim} may be used directly.
Translating~\eqref{eq:logrho_nondim} and~\eqref{eq:p_nondim} in 2D gives the
following:
\begin{align}
  \left(\log\rho\right)\left(R_2\right)
  &=
  FIXME STARTHERE
  \int_{R_1}^{R_2}
  \, \mathrm{d}r
