...
radius.  Such a solution may be transformed into conserved variables and then
mapped from $\left(r,\theta\right)$ into $\left(x,y\right)$ coordinates.  Then
conserved state and its derivatives may be collected from some constant $y$
line.  Curve fits are created from that data and supplied as base flow
coefficients within simulation restart files.
...
\lstinputlisting[language=Octave,columns=fixed,firstline=4,
                 basicstyle={\footnotesize\sffamily},frame=single]
                {notebooks/FPG_BaseFlow_Problem.m}

\begin{figure}
  \centering
  \includegraphics[width=0.65\textwidth]{notebooks/nozzle_subsonic}
  \\
  \includegraphics[width=0.65\textwidth]{notebooks/nozzle_supersonic}
  \caption{Solutions produced with the sample \texttt{Octave} code}
\end{figure}
