...
,
\end{align}
permits integrating using $u$ via classical ODE techniques whenever a boundary
condition on either $R_1$ or $R_2$ is supplied.
...
\end{align}
%
The $\Mach/\Mach[0]{}$ factors on $p$ and $a$ prepare them for use within a
...
state, $\rho{}a^2=\gamma_{0} p$.  However, at sufficiently large radii when
care is taken in supplying initial conditions, it is possible to achieve
solutions in which $\rho a^2 \approx \gamma_{0} p$.

More specifically, the Cartesian flow profile should be computed from the
radial solution on the line segment spanning points $\left(R_0,0\right)$ and
...
inherent in the second quadrant relative to the first quadrant.
