...
solve such problems.  An implementation handling each equation in turn appears
in Listing~\ref{lst:octave_nozzle1}.  While crisp, it is apparent that the
discrete solutions for $\rho$ and $p$ do not enjoy the same accuracy guarantees
as the velocity $u$.  By coupling $u'$, $\left(\log\rho\right)'$, and $p'$ one
may easily achieve higher order behavior across all three scalars as shown in
Listing~\ref{lst:octave_nozzle}.  Listing~\ref{lst:octave_nozzle_usecase}
demonstrates using this higher order implementation to solve concrete problems
producing the solutions depicted in Figure~\ref{fig:sample_solns}.
...
\lstinputlisting[language=Octave,columns=fixed,float,
                 ...
                 implementation\label{lst:octave_nozzle_usecase}},
                 basicstyle={\footnotesize\sffamily},frame=single]
...
\begin{figure}[p]
  ...
  \caption{
      ...
      Sample radial nozzle solutions.  The subsonic solution
      flows from right-to-left while the supersonic one flows from
      left-to-right.
  }
\end{figure}
...
automatically produce solution data at $\left(R_0,\delta^\ast\right)$.  An
implementation of this inner solve appears in
Listing~\ref{lst:octave_nozzle_edge}.  After optimization completes, the full
profile requiring taking $R_2 = \sqrt{R_0^2 + L_y^2}$ can be computed by
...
\textcolor{red}{TODO Refer to and discuss
Listing~\ref{lst:octave_baseflow_residmin}.}

\lstinputlisting[language=Octave,columns=fixed,float,
                 caption={\texttt{baseflow\_residmin.m}: A driver targeting
                 quantities of interest\label{lst:octave_baseflow_residmin}},
                 basicstyle={\footnotesize\sffamily},frame=single]
                {notebooks/baseflow_residmin.m}
