...
\section{Computing thermodynamic state from a velocity potential solution}

Having hypothetically obtained some $\vec{u}=\vec{\nabla}\phi$
satisfying~\eqref{eq:cpfgibbs_nondim}, $a$, $\rho$, and $p$ are often of
interest.  Nondimensionalizing~\eqref{eq:stagnation_sound} permits direct
...
.
\end{align}
Combining dimensional~\eqref{eq:gradrho_gradp_relationship} and~\eqref{eq:momentum},
\begin{align}
  \frac{\vec{\nabla}\rho}{\rho}
  &=
  - \frac{\vec{\nabla}\vec{u}^2}{2a^2}
