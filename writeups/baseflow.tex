...
International Space Station return, fully-laminar Crew Exploration Vehicle
(CEV) heat shield simulations were performed by Paul Bauman of The Center for
Predictive Engineering and Computational Science using the FIN-S hypersonic
flow code.  Scenario information was gathered along surface-normal rays on the
symmetry plane which are marked with light lines in
...
\begin{figure}[p]
  \centering
  \includegraphics[width=0.9\textwidth]{symplanenorm}
  \caption{
...
$\Mach[e]{}$ are taken from the boundary layer edge a distance $\delta$ from
...
$a_e^2$ when $a_w^2=T_w=1$ is used to fix an isothermal wall condition.
...
these values from a viscous CEV simulation, $p_{e,\xi}$ is the only quantity
with a direct inviscid analog once a thickness length scale $\delta$ is
supplied.
