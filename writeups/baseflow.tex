...
radius.  \textcolor{red}{TODO} The solution is then mapped from
$\left(r,\theta\right)$ into $\left(x,y\right)$ coordinates and a base flow
profile taken from some constant $x$ line segment.  The segment chosen, as well
as the radial problem boundary conditions used, are taken to match some
boundary layer edge state of interest.  After matching, conserved state and its
derivatives are computed from the primitive results.  Curve fits are created
from that data and supplied as base flow coefficients within simulation restart
files.
...
Sample solutions computed by the code shown in Listing~\ref{lst:octave_nozzle}
appear in Figure~\ref{fig:sample_solns}.

\lstinputlisting[language=Octave,columns=fixed,firstline=4,float=p,
...
\section{Mapping radial solution and matching conditions of interest}
...
a constant $x$ line segment obtained from the radially-varying solution.
Referring to Figure~\eqref{fig:mapping_geometry}, the solution
$u\!\left(R\right)$ with $p\!\left(R\right)$ and $a\!\left(R\right)$ is valid
for $R\in\left[R_1,R_2\right]$.  In particular, for some $R_0$ satisfying $R_0
\leq R_2 - L_y$, the line segment between Cartesian points $\left(R_0,0\right)$
and $\left(R_0,L_y\right)$ is inside the domain.  At some height of interest
within the profile, say $\delta^\ast$, one can evaluate
\begin{align}
    u\cdot\hat{x}
  \bigr|_{\sqrt{R_0^2 + {\delta^\ast}^2}}
  &=
  \frac{R_0}{\sqrt{R_0^2+{\delta^\ast}^2}}
  u\!\left(\sqrt{R_0^2 + {\delta^\ast}^2}\right)
\\
  \left.
...
\end{align}
to interrogate the solution's nature at $\left(R_0, \delta^\ast\right)$.  The
temperature result relies on that, in the special case of a thermally and
calorically perfect gas with dimensional equation of state $p = \rho R T$,
choices~\eqref{eq:nondimensionalization} imply $a^2 = T$ holds
nondimensionally.
