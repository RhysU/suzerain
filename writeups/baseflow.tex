...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.28\textwidth]{nozzle_schematic}
  \caption{
...
$\delta$, one can evaluate
\begin{align}
  \Mach[e]{}
  &\equiv
  \left.
    \frac{u_0 u\cdot\hat{\xi}}{a_0 a}
  \right|_{\sqrt{R_0^2 + \delta^2}}
  =
  \frac{\Mach R_0 \, \left|u\!\left(\sqrt{R_0^2 + \delta^2}\right)\right|}
       {\sqrt{R_0^2+\delta^2} \, a\!\left(\sqrt{R_0^2 + \delta^2}\right)}
\end{align}
\begin{align}
  p_{e,\xi}
  &\equiv
...
% \right|_{\sqrt{R_0^2 + \delta^2}}
  =
  \frac{\operatorname{sgn}(u)\sqrt{R_0^2+\delta^2}}{\Mach^2 R_0}
  \,
  p'\!\left(\sqrt{R_0^2 + \delta^2}\right)
\end{align}
to interrogate the solution's nondimensional nature at $\left(R_0,
\delta\right)$.  Here, $\hat{\xi}$ denotes $\hat{x}$ with the streamwise
...
$L_y$, $\delta$, and $\gamma_0$.  Parameter $p\!\left(R_1\right)$ is
...
is sped up by taking $R_2 = \sqrt{R_0^2 + \delta^2}$ which causes the
implementation shown in Listing~\ref{lst:octave_nozzle} to automatically
produce data at $\left(R_0,\delta\right)$.  The \texttt{nonlim\_resid}
