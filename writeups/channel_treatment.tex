...
\begin{align}
  ...
  \partial_{t} e      &= R_{e}\!\left(\rho,\vec{m},e\right)
                       + \mbox{Ma}^{2} \vec{a}_{m}(t) \cdot \vec{m}
\end{align}
\end{subequations}
...
form of the uniform body acceleration $\vec{a}_{m}$ necessary to drive an
isothermal channel problem similar to Coleman et
...
\end{align}
also holds at the wall.  Finally, we find $\vec{a}_{m} =
\left(a_{m_x},0,0\right)^{\textrm{T}}$ such that the bulk streamwise momentum
...
\begin{align}
  ...
  &= \frac{1}{2\delta} \int_{0}^{2\delta}
     \left[  R_{m_x}\!\left(\cdot,y,\cdot\right)
           + \rho\!\left(\cdot,y,\cdot\right)  a_{m_x}
     \right] \, dy
  \\
  &= \frac{1}{2\delta} \int_{0}^{2\delta}
     R_{m_x}\!\left(\cdot,y,\cdot\right) \, dy + \bar{\rho} a_{m_x}
   = 0
\end{align}
Rearranging we find $a_{m_x} = - \frac{1}{\bar{\rho}2\delta}
...
\begin{enumerate}
  ...
  \item Apply $\rho a_{m_x}$ by updating
        $R_{m_x}\!\left(\cdot,y,\cdot\right) -{}\!\!\!{}=
        \alpha \, \rho\!\left(\cdot,y,\cdot\right)$ for $y\neq0,2\delta$.
  \item Apply $\mbox{Ma}^{2}\vec{a}_{m}\cdot\vec{m} = \mbox{Ma}^{2} a_{m_x}
        m_x$ by updating $R_{e}\!\left(\cdot,y,\cdot\right) -{}\!\!\!{}=
        \mbox{Ma}^{2} \alpha \, m_{x}\!\left(\cdot,y,\cdot\right)$ for
        $y\neq0,2\delta$.
\end{enumerate}
...
isothermal channel conditions.  Note that $a_{m_x}$ is not applied at
...
associated substep energy contribution $a_{m_x} m_{x}$ can be determined.
