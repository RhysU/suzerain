...
\subsection{Expansions about reference values}
...
Because derivatives, particularly second derivatives, give rise to the most
restrictive eigenvalues for timestepping, we focus on how they enter the full
operator.  We take the quantities from
sections~\ref{velocity_derivative_expansions}
and~\ref{separate_first_section_deriv} and expand them about reference states.
For example, $\rho^{-1}\nabla{}m = \left(\rho^{-1} -
\reference{\rho^{-1}}\right)\nabla{}m + \reference{\rho^{-1}}\nabla{}m$ where
$\reference{\rho^{-1}}$ indicates the term $\rho^{-1}$ evaluated at some given,
fixed state.  This approach separates each quantity into a nonlinear portion
contribution to be treated explicitly and a linear, derivative-oriented
contribution that satisfies our timestepper's implicit operator needs:
\begin{subequations}
\begin{align}
\nabla\cdot\frac{m}{\rho}
&=
     \left(\rho^{-1}-\reference{\rho^{-1}}\right)\nabla\cdot{}m
  &
  &{}+\reference{\rho^{-1}}\nabla\cdot{}m
\\
  &{}- \left(\rho^{-2}m-\reference{\rho^{-2}m}\right)\cdot\nabla\rho
  &
  &{}- \reference{\rho^{-2}m}\cdot\nabla\rho
\\
\nabla{}\frac{m}{\rho}
&=
       \left(\rho^{-1}-\reference{\rho^{-1}}\right)\nabla{}m
  ...
  &{}+ \reference{\rho^{-1}}\nabla{}m
\\
  &{}- \left(\rho^{-2}m-\reference{\rho^{-2}m}\right)\otimes\nabla\rho
  &
  &{}- \reference{\rho^{-2}m}\otimes\nabla\rho
\\
\symmetricpart{\nabla\frac{m}{\rho}}
&=
       \left(\rho^{-1}-\reference{\rho^{-1}}\right)\symmetricpart{\nabla{}m}
  &
  &{}+ \reference{\rho^{-1}}\symmetricpart{\nabla{}m}
\\
  &{}- \symmetricpart{
       \left(\rho^{-2}m-\reference{\rho^{-2}m}\right)\otimes\nabla\rho}
  &
  &{}- \symmetricpart{\reference{\rho^{-2}m} \otimes\nabla\rho}
\\
\Delta\frac{m}{\rho}
&=
       \left(\rho^{-1}-\reference{\rho^{-1}}\right)\Delta{}m
  &
  &{}+ \reference{\rho^{-1}}\Delta{}m
\\
  &{}+ 2\left(\rho^{-3}m\nabla\rho - \reference{\rho^{-3}m\nabla\rho}\right)
       \cdot\nabla\rho
  &
  &{}+ 2\reference{\rho^{-3}m\nabla\rho}\cdot\nabla\rho
\\
  &{}- \left(\rho^{-2}m-\reference{\rho^{-2}m}\right)\Delta\rho
  &
  &{}- \reference{\rho^{-2}m}\Delta\rho
\\
  &{}- 2\nabla{}m\left(
         \rho^{-2}\nabla\rho - \reference{\rho^{-2}\nabla\rho}
       \right)
  &
  &{}- 2\nabla{}m\reference{\rho^{-2}\nabla\rho}
\\
\nabla\nabla\cdot\frac{m}{\rho} &=
       \left(\rho^{-1}-\reference{\rho^{-1}}\right)\nabla\nabla\cdot{}m
  &
...
\\
  ...
  &{}- \reference{\rho^{-2}\nabla\rho}\nabla\cdot{}m
\\
  &{}- \nabla\nabla\rho\left(\rho^{-2}m - \reference{\rho^{-2}m}\right)
  &
  &{}- \nabla\nabla\rho\reference{\rho^{-2}m}
\\
  &{}- \trans{\nabla{}m}\left(
          \rho^{-2}\nabla\rho - \reference{\rho^{-2}\nabla\rho}
       \right)
