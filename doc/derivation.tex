...
For example, $\rho^{-1}\Delta{}m = \lessreference{\rho^{-1}}\Delta{}m +
\reference{\rho^{-1}}\Delta{}m$ where $\reference{\rho^{-1}}$ indicates the
term $\rho^{-1}$ evaluated at some given, fixed state.  This approach separates
each quantity into a nonlinear portion contribution to be treated explicitly
and a linear, derivative-oriented contribution that satisfies our timestepper's
...
&=
  ...
  &{}+ \reference{\rho^{-1}}\nabla{}m
\\
  &{}- \lessreference{\rho^{-2}m}\otimes\nabla\rho
  &
  &{}- \reference{\rho^{-2}m}\otimes\nabla\rho
\\
\symmetricpart{\nabla\frac{m}{\rho}}
&=
       \lessreference{\rho^{-1}}\symmetricpart{\nabla{}m}
  &
  &{}+ \reference{\rho^{-1}}\symmetricpart{\nabla{}m}
\\
  &{}- \symmetricpart{\lessreference{\rho^{-2}m}\otimes\nabla\rho}
  &
  &{}- \symmetricpart{\reference{\rho^{-2}m} \otimes\nabla\rho}
\\
\Delta\frac{m}{\rho}
&=
       \lessreference{\rho^{-1}}\Delta{}m
  &
  &{}+ \reference{\rho^{-1}}\Delta{}m
\\
  &{}+ 2\lessreference{\rho^{-3}m\nabla\rho}\cdot\nabla\rho
  &
  &{}+ 2\reference{\rho^{-3}m\nabla\rho}\cdot\nabla\rho
\\
  &{}- \lessreference{\rho^{-2}m}\Delta\rho
  &
  &{}- \reference{\rho^{-2}m}\Delta\rho
\\
  &{}- 2\nabla{}m\lessreference{\rho^{-2}\nabla\rho}
  &
...
\\
  ...
  &{}- \reference{\rho^{-2}\nabla\rho}\nabla\cdot{}m
\\
  &{}- \nabla\nabla\rho\lessreference{\rho^{-2}m}
  &
