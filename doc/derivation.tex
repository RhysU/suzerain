...
differentiation severely abates high frequency modes (see~\cite{Kwok2001}
figures 2 and 3).
...
first and second derivative applications wholly separate.  Doing so will ensure
that these three terms have the appropriate diffusive effects on high frequency
content.  We start by expanding these three nonlinear terms:
\begin{subequations}
\begin{align}
  \nabla\cdot\tau
  &=
...
\end{align}
\end{subequations}
We then expand the non-state derivatives appearing above
\begin{subequations}
\begin{align}
  \nabla{}p &= (\gamma-1)\left[
        \nabla{}e
...
\end{align}
\end{subequations}
where we have employed the shorthand $m^2=m\cdot{}m$ and
$\left(\nabla\rho\right)^2=\nabla\rho\cdot\nabla\rho$.

\begin{table}[h]
\centering
\renewcommand{\arraystretch}{1.25} % Adds whitespace between rows
...
\end{tabular}
\caption{
  State variable derivatives required to compute quantities in physical space
  without repeated first derivative applications
}
\label{tab:nofirstderivcost}
\end{table}
