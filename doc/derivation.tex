...
timestepping, we focus on how they enter the full operator.  We take the
quantities from sections~\ref{velocity_derivative_expansions}
...
contribution to be treated explicitly and a linear, second derivative-oriented
contribution that satisfies our timestepper's implicit operator needs:
\begin{subequations}
\begin{align}
\Delta\frac{m}{\rho} = &\phantom{{}+}
...
\Delta{}p = &\phantom{{}+}
   ...
   {}- \left(\gamma-1\right)\rho^{-1}m\cdot\Delta{}m
     + \frac{1}{2}\left(\gamma-1\right)\rho^{-2}m^2 \Delta\rho
\\
