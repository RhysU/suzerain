...
/**
 * Find flow state \f$u\left(R\right)\f$, \f$\rho\left(R\right)\f$, and
 * \f$p\left(R\right)\f$ and radial extents \f$\left[R_0, R\right]\f$ reproducing
 ...
 * for a slow growth baseflow definition per \c largo_state.
 *
 * @param[in ] s A solution generated by \ref suzerain_radialflow_solver.
 * @param[in ] i An indexed location within \c s in range <tt>[0,s->size)</tt>.
