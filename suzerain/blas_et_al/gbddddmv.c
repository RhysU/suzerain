...
#define BOOST_PP_FILENAME_1 <suzerain/blas_et_al/gbddddmv.c>
...
#define STATIC   static
#define FUNCTION suzerain_gbddddmv_internal_ssc
#define TYPE_D   float
#define TYPE_A   float
#define TYPE_X   float
...
#define STATIC   static
#define FUNCTION suzerain_gbddddmv_internal_ddz
#define TYPE_D   double
#define TYPE_A   double
#define TYPE_X   double
        ...
        const int incy)
{
    ...
    if (kl == ku) {
        switch (kl) {
            BOOST_PP_REPEAT_FROM_TO(FIXEDBW_LOWER, BOOST_PP_INC(FIXEDBW_UPPER),
                                    FIXEDBW_CASE, suzerain_gbddddmv_internal_ssc)
        }
    }

    // ...otherwise employ a general bandwidth implementation
    return suzerain_gbddddmv_internal_ssc(trans, n, kl, ku,
                                          alpha0, d0, ldd0,
                                          alpha1, d1, ldd1,
                                          alpha2, d2, ldd2,
                                          alpha3, d3, ldd3,
        ...
        const int incy)
{
    ...
    if (kl == ku) {
        switch (kl) {
            BOOST_PP_REPEAT_FROM_TO(FIXEDBW_LOWER, BOOST_PP_INC(FIXEDBW_UPPER),
                                    FIXEDBW_CASE, suzerain_gbddddmv_internal_scc)
        }
    }

    // ...otherwise employ a general bandwidth implementation
    return suzerain_gbddddmv_internal_scc(trans, n, kl, ku,
                                          alpha0, d0, ldd0,
                                          alpha1, d1, ldd1,
                                          alpha2, d2, ldd2,
                                          alpha3, d3, ldd3,
        ...
        const int incy)
{
    ...
    if (kl == ku) {
        switch (kl) {
            BOOST_PP_REPEAT_FROM_TO(FIXEDBW_LOWER, BOOST_PP_INC(FIXEDBW_UPPER),
                                    FIXEDBW_CASE, suzerain_gbddddmv_internal_ddz)
        }
    }

    // ...otherwise employ a general bandwidth implementation
    return suzerain_gbddddmv_internal_ddz(trans, n, kl, ku,
                                          alpha0, d0, ldd0,
                                          alpha1, d1, ldd1,
                                          alpha2, d2, ldd2,
                                          alpha3, d3, ldd3,
        ...
        const int incy)
{
    ...
    if (kl == ku) {
        switch (kl) {
            BOOST_PP_REPEAT_FROM_TO(FIXEDBW_LOWER, BOOST_PP_INC(FIXEDBW_UPPER),
                                    FIXEDBW_CASE, suzerain_gbddddmv_internal_dzz)
        }
    }

    // ...otherwise employ a general bandwidth implementation
    return suzerain_gbddddmv_internal_dzz(trans, n, kl, ku,
                                          alpha0, d0, ldd0,
                                          alpha1, d1, ldd1,
                                          alpha2, d2, ldd2,
                                          alpha3, d3, ldd3,
...
#define STATIC   static
#define FUNCTION BOOST_PP_CAT(suzerain_gbddddmv_internal_ssc, \
                              BOOST_PP_ITERATION())
#define TYPE_D   float
#define TYPE_A   float
#define TYPE_X   float
...
#define STATIC   static
#define FUNCTION BOOST_PP_CAT(suzerain_gbddddmv_internal_ddz, \
                              BOOST_PP_ITERATION())
#define TYPE_D   double
#define TYPE_A   double
#define TYPE_X   double
