...
/**
 * Compute an approximation to the maximum stable time step according to a
 * three-dimensional mixed convective-diffusive term.   This surrogate is a
 * model for the viscous work term in the Navier-Stokes total energy equation.
 * It appears in neither Wai Y. Kwok nor Stephen Guarini's thesis.
 *
 * The surrogate is been introduced because implicit treatment of the acoustic
 * and viscous terms but explicit treatment of the viscous work term allows
 * simulations with subsonic bulk velocities to take diffusive-limited time
 * steps too large for numerical stability.  Reducing the safety factor (for
 * example, using 0.55) for subsonic simulations restores stability but is
 * inefficient for supersonic flows.  Manually requiring the user to adjust
 * this stability knob is cumbersome and error prone.  Instead, we add this
 * stability criterion which should only be practically limiting in the
 * subsonic case:
 * \f[
 *   \mbox{max}\!\left(
 *     \frac{\nu}{\mbox{Re}},
 ...
 *     + \frac{u_z}{\Delta{}z^{3}}
 *   \right)
 *   {\Delta{}t}^2 \leq \left|\lambda_{R}\Delta_{}t\right|_{\mbox{max}}
 * \f]
 * where the third power of grid spacing and second power of time step are
 * necessary to achieve dimensional consistency.  The maximum pure real
 * eigenvalue magnitude, \f$\left|\lambda_{R}\Delta{}t\right|_{\mbox{max}}\f$,
 * is a feature of the chosen timestepping method.
 *
 * @param u_x              Velocity in the X direction \f$u_{x}\f$
 * @param one_over_delta_x Inverse local X grid spacing \f$1/\Delta{}x\f$
 * @param u_y              Velocity in the Y direction \f$u_{y}\f$
 ...
 *
 * @return The maximum stable time step \f$\Delta{}t\f$ according to
 *         this ad hoc viscous work stability criterion.
 */
template<typename FPT>
FPT viscouswork_stability_criterion(
        const FPT u_x,
        const FPT one_over_delta_x,
        const FPT u_y,
        const FPT one_over_delta_y,
