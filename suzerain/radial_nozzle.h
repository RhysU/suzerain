 ...
 * streamwise velocity always has positive sign.
 *
 * Locations <tt>s->state[j].R</tt> are taken to be along an \f$x\f$-oriented
 * line segment beginning at Cartesian position \f$\left(R_0,0\right)\f$ and
 * extending until \f$\left(R_0,\delta\right)\f$ where \f$\delta=\sqrt{R_i^2 -
 * R_0^2}\f$.  Given this geometric assumption, only the \f$x\f$-component of the
 * radial velocity \f$u\left(R\right)\f$ contributes to \f$\mbox{Ma}_e\f$.
 *
 * @param s A solution generated by \ref suzerain_radial_nozzle_solver.
 * @param i An indexed location within \c s in the range <tt>[0,s->size)</tt>.
 *
 ...
 *   \right|_{R=\sqrt{R_0^2 + \delta^2}}
 * \f}
 * defining \f$R_0\f$ and \f$\delta\f$ per <tt>s->state.R[i]</tt> and the same
 * geometric assumptions described for \ref suzerain_radial_nozzle_qoi_pexi.
 *
 * @param s A solution generated by \ref suzerain_radial_nozzle_solver.
 * @param i An indexed location within \c s in the range <tt>[0,s->size)</tt>.
 *
