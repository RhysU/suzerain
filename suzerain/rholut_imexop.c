        ...
        complex_double *out_rhoe)
{
    ...
    //
    // The suzerain_bsplineop_* routines account for the
    // suzerain_bsplineop_workspace storing the transpose of the operators, but
    // we need to account for it manually using 'T' when we invoke
    // suzerain_blaseext_* routines.
...
#   define REF(quantity) r->quantity, ld->quantity
    ...
    if (in_rho ) {  // Accumulate density terms into out_rho
        ...
        if (in_rhov) suzerain_bsplineop_accumulate_complex(D1,nrhs,
                -phi,     in_rhov, inc, n, 1.0, out_rho, inc, n, w);
    ...
    if (in_rhou) {  // Accumulate X momentum terms into out_rhou
        ...
        if (in_rho) {
            ...
            suzerain_blasext_zgbddmv_d('T', n, w->kl[D1], w->ku[D1],
                ...
                w->D_T[D1], w->ld, in_rho, inc, 1.0, out_rhou, inc);

            suzerain_blasext_zgbdmv_d('T', n, w->kl[D2], w->ku[D2],
                -phi*invRe,                REF(nuux),
                w->D_T[D2], w->ld, in_rho, inc, 1.0, out_rhou, inc);
        }
        ...
        /* in_rhou */ {
            suzerain_blasext_zgbdddmv_d('T', n, w->kl[M], w->ku[M],
                ...
                w->D_T[M],  w->ld, in_rhou, inc, 1.0, out_rhou, inc);

            suzerain_blasext_zgbdmv_d('T', n, w->kl[D1], w->ku[D1],
                -phi,                      REF(uy),
                w->D_T[D1], w->ld, in_rhou, inc, 1.0, out_rhou, inc);

            suzerain_blasext_zgbdmv_d('T', n, w->kl[D2], w->ku[D2],
                phi*invRe,                 REF(nu),
                w->D_T[D2], w->ld, in_rhou, inc, 1.0, out_rhou, inc);
        }

        if (in_rhov) {
            suzerain_blasext_zgbdmv_d('T', n, w->kl[M], w->ku[M],
                phi*gm1*ikm,               REF(uy),
                w->D_T[M],  w->ld, in_rhov, inc, 1.0, out_rhou, inc);

            suzerain_blasext_zgbddmv_d('T', n, w->kl[D1], w->ku[D1],
                -phi,                      REF(ux),
                phi*ap13*invRe*ikm,        REF(nu),
                w->D_T[D1], w->ld, in_rhov, inc, 1.0, out_rhou, inc);
        }
        ...
        if (in_rhow) {
            suzerain_blasext_zgbdddmv_d('T', n, w->kl[M], w->ku[M],
                ...
                w->D_T[M],  w->ld, in_rhow, inc, 1.0, out_rhou, inc);
        }
    ...
    if (in_rhov) {  // Accumulate Y momentum terms into out_rhov
        ...
        if (in_rho) {
            ...
            suzerain_blasext_zgbdmv_d('T', n, w->kl[D2], w->ku[D2],
                ...
                w->D_T[D2], w->ld, in_rho, inc, 1.0, out_rhov, inc);
        }

        if (in_rhou) {
            suzerain_blasext_zgbdmv_d('T', n, w->kl[M], w->ku[M],
                -phi*ikm,             REF(uy),
                w->D_T[M],   w->ld, in_rhou, inc, 1.0, out_rhov, inc);

            suzerain_blasext_zgbddmv_d('T', n, w->kl[D1], w->ku[D1],
                phi*gm1,              REF(ux),
                phi*ap13*invRe*ikm,   REF(nu),
                w->D_T[D1],  w->ld, in_rhou, inc, 1.0, out_rhov, inc);
        }
        ...
        /* in_rhov */ {
            suzerain_blasext_zgbdddmv_d('T', n, w->kl[M], w->ku[M],
                ...
                w->D_T[M],  w->ld, in_rhov, inc, 1.0, out_rhov, inc);

            suzerain_blasext_zgbdmv_d('T', n, w->kl[D1], w->ku[D1],
                phi*gm3,              REF(uy),
                w->D_T[D1], w->ld, in_rhov, inc, 1.0, out_rhov, inc);

            suzerain_blasext_zgbdmv_d('T', n, w->kl[D2], w->ku[D2],
                phi*ap43*invRe,       REF(nu),
                w->D_T[D2], w->ld, in_rhov, inc, 1.0, out_rhov, inc);
        }

        if (in_rhow) {
            suzerain_blasext_zgbdmv_d('T', n, w->kl[M], w->ku[M],
                -phi*ikn,             REF(uy),
                w->D_T[M],   w->ld, in_rhow, inc, 1.0, out_rhov, inc);

            suzerain_blasext_zgbddmv_d('T', n, w->kl[D1], w->ku[D1],
                phi*gm1,              REF(uz),
                phi*ap13*invRe*ikn,   REF(nu),
                w->D_T[D1],  w->ld, in_rhow, inc, 1.0, out_rhov, inc);
        }
    ...
    if (in_rhow) {  // Accumulate Z momentum terms into out_rhow
        ...
        if (in_rho) {
            ...
            suzerain_blasext_zgbddmv_d('T', n, w->kl[D1], w->ku[D1],
                ...
                w->D_T[D1], w->ld, in_rho, inc, 1.0, out_rhow, inc);

            suzerain_blasext_zgbdmv_d('T', n, w->kl[D2], w->ku[D2],
                -phi*invRe,                REF(nuuz),
                w->D_T[D2], w->ld, in_rho, inc, 1.0, out_rhow, inc);
        }
        ...
        if (in_rhou) {
            suzerain_blasext_zgbdddmv_d('T', n, w->kl[M], w->ku[M],
                ...
                -phi*ikm,                  REF(uz),
                -phi*ap13*invRe*km*kn,     REF(nu),
                w->D_T[M],  w->ld, in_rhou, inc, 1.0, out_rhow, inc);
        }

        if (in_rhov) {
            suzerain_blasext_zgbdmv_d('T', n, w->kl[M], w->ku[M],
                phi*gm1*ikn,               REF(uy),
                w->D_T[M],  w->ld, in_rhov, inc, 1.0, out_rhow, inc);

            suzerain_blasext_zgbddmv_d('T', n, w->kl[D1], w->ku[D1],
                -phi,                      REF(uz),
                phi*ap13*invRe*ikn,        REF(nu),
                w->D_T[D1], w->ld, in_rhov, inc, 1.0, out_rhow, inc);
        }
        ...
        /* in_rhow */ {
            suzerain_blasext_zgbdddmv_d('T', n, w->kl[M], w->ku[M],
                ...
                w->D_T[M],  w->ld, in_rhow, inc, 1.0, out_rhow, inc);

            suzerain_blasext_zgbdmv_d('T', n, w->kl[D1], w->ku[D1],
                -phi,                      REF(uy),
                w->D_T[D1], w->ld, in_rhow, inc, 1.0, out_rhow, inc);

            suzerain_blasext_zgbdmv_d('T', n, w->kl[D2], w->ku[D2],
                phi*invRe,                 REF(nu),
                w->D_T[D2], w->ld, in_rhow, inc, 1.0, out_rhow, inc);
        }
    ...
    if (in_rhoe) {  // Accumulate total energy terms into out_rhoe
        ...
        /* in_rhoe */ {
            suzerain_blasext_zgbdddmv_d('T', n, w->kl[M], w->ku[M],
                ...
                w->D_T[M],  w->ld, in_rhoe, inc, 1.0, out_rhoe, inc);

            suzerain_blasext_zgbdmv_d('T', n, w->kl[D1], w->ku[D1],
                -phi*s->gamma,               REF(uy),
                w->D_T[D1], w->ld, in_rhoe, inc, 1.0, out_rhoe, inc);

            suzerain_blasext_zgbdmv_d('T', n, w->kl[D2], w->ku[D2],
                phi*ginvRePr,                REF(nu),
                w->D_T[D2], w->ld, in_rhoe, inc, 1.0, out_rhoe, inc);
        }
