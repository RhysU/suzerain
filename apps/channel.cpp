        ...
        boost::ptr_map<std::string,Eigen::ArrayXXr> &samples)
{
    ...
    // Clear any lingering data in samples and prepare the key/value pairs.
...
#define DECLARE_SAMPLE(q,n)                           \
    ...
    DECLARE_SAMPLE(rhoe,                     1);
    DECLARE_SAMPLE(mu,                       1);
    DECLARE_SAMPLE(u,                        3);
    DECLARE_SAMPLE(sym_rho_grad_u,           6);
    DECLARE_SAMPLE(rho_grad_T,               3);
    DECLARE_SAMPLE(tau_colon_grad_u,         1);
    DECLARE_SAMPLE(tau,                      6);
    DECLARE_SAMPLE(tau_u,                    3);
    DECLARE_SAMPLE(p_div_u,                  1);
    DECLARE_SAMPLE(rho_u_otimes_u,           6);
    DECLARE_SAMPLE(rho_u_otimes_u_otimes_u, 10);
    DECLARE_SAMPLE(rho_T_u,                  3);
    DECLARE_SAMPLE(mu_S,                     6);
    DECLARE_SAMPLE(mu_div_u,                 1);
    DECLARE_SAMPLE(mu_grad_T,                3);
#undef DECLARE_SAMPLE
#undef STRINGIFY
    ...
    bar_rhou.col(2) = Map<VectorXc>(swave[ndx::rhow].origin(), Ny).real();
    bar_rhoe        = Map<VectorXc>(swave[ndx::rhoe].origin(), Ny).real();
    ...
    for (int j = dgrid.local_physical_start.y();
        ...
        // Accumulators for each mean quantity determined in physical space.
        // Declared within Y loop so they'll be reset on each Y iteration.
        accumulator_type sum_mu;
        accumulator_type sum_u[3];
        accumulator_type sum_sym_rho_grad_u[6];
        accumulator_type sum_rho_grad_T[3];
        accumulator_type sum_tau_colon_grad_u;
        accumulator_type sum_tau[6];
        accumulator_type sum_tau_u[3];
        accumulator_type sum_p_div_u;
        accumulator_type sum_rho_u_otimes_u[6];
        accumulator_type sum_rho_u_otimes_u_otimes_u[10];
        accumulator_type sum_rho_T_u[3];
        accumulator_type sum_mu_S[6];
        accumulator_type sum_mu_div_u;
        accumulator_type sum_mu_grad_T[3];
        ...
        for (int k = dgrid.local_physical_start.z();
            ...
            for (int i = dgrid.local_physical_start.x();
                ...
                sum_tau_colon_grad_u((tau.transpose()*grad_u).trace());
        ...
        // Move sums from accumulators into MPI-reduction-ready storage
#define MOVE_SUM_ARRAY(q)                                                    \
        for (std::size_t l = 0; l < sizeof(sum_##q)/sizeof(sum_##q[0]); ++l) \
            bar_##q(j, l) = acc::sum(sum_##q[l])

        bar_mu(j,0) = acc::sum(sum_mu);
        MOVE_SUM_ARRAY(u);
        MOVE_SUM_ARRAY(sym_rho_grad_u);
        MOVE_SUM_ARRAY(rho_grad_T);
        bar_tau_colon_grad_u(j,0) = acc::sum(sum_tau_colon_grad_u);
        MOVE_SUM_ARRAY(tau);
        MOVE_SUM_ARRAY(tau_u);
        bar_p_div_u(j,0) = acc::sum(sum_p_div_u);
        MOVE_SUM_ARRAY(rho_u_otimes_u);
        MOVE_SUM_ARRAY(rho_u_otimes_u_otimes_u);
        MOVE_SUM_ARRAY(rho_T_u);
        MOVE_SUM_ARRAY(mu_S);
        bar_mu_div_u(j,0) = acc::sum(sum_mu_div_u);
        MOVE_SUM_ARRAY(mu_grad_T);
#undef MOVE_SUM_ARRAY

    } // end Y

    // Reduce sums and divide to obtain mean quantities on all ranks.  Done as
    // in-place Allreduce operations to optimize memory usage rather than as
    // out-of-place reduces to rank zero to optimize communication.  Done
    // one-at-a-time to avoid packing memory overhead and ease adding
    // additional quantities rather than all-at-once to reduce wall time.

#define ALLREDUCE_MEAN(q)                           \
    SUZERAIN_MPICHKR(MPI_Allreduce(MPI_IN_PLACE,    \
            bar_##q.data(), bar_##q.size(),         \
            suzerain::mpi::datatype<real_t>::value, \
            MPI_SUM, MPI_COMM_WORLD));              \
    bar_##q /= (   dgrid.global_physical_extent.x() \
                 * dgrid.global_physical_extent.z());

    ALLREDUCE_MEAN(mu);
    ALLREDUCE_MEAN(u);
    ALLREDUCE_MEAN(sym_rho_grad_u);
    ALLREDUCE_MEAN(rho_grad_T);
    ALLREDUCE_MEAN(tau_colon_grad_u);
    ALLREDUCE_MEAN(tau);
    ALLREDUCE_MEAN(tau_u);
    ALLREDUCE_MEAN(p_div_u);
    ALLREDUCE_MEAN(rho_u_otimes_u);
    ALLREDUCE_MEAN(rho_u_otimes_u_otimes_u);
    ALLREDUCE_MEAN(rho_T_u);
    ALLREDUCE_MEAN(mu_S);
    ALLREDUCE_MEAN(mu_div_u);
...
#undef ALLREDUCE_MEAN
    ...
    SUZERAIN_MPICHKR(MPI_Bcast(bar_rho.data(), bar_rho.size(),
                suzerain::mpi::datatype<real_t>::value, 0, MPI_COMM_WORLD));
    SUZERAIN_MPICHKR(MPI_Bcast(bar_rhou.data(), bar_rhou.size(),
                suzerain::mpi::datatype<real_t>::value, 0, MPI_COMM_WORLD));
    SUZERAIN_MPICHKR(MPI_Bcast(bar_rhoe.data(), bar_rhoe.size(),
                suzerain::mpi::datatype<real_t>::value, 0, MPI_COMM_WORLD));
}
