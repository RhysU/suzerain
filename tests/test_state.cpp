...
static void load223(
      typename suzerain::InterleavedState<FPT> &state,
      const Scale scale,
      typename boost::enable_if<boost::is_floating_point<FPT> >::type *dummy = 0)
{
    SUZERAIN_UNUSED(dummy);
    BOOST_REQUIRE_EQUAL(state.variable_count, 2);
    BOOST_REQUIRE_EQUAL(state.vector_length, 2);
...
static void verify223(
      const typename suzerain::InterleavedState<FPT> &state,
      const Scale scale,
      typename boost::enable_if<boost::is_floating_point<FPT> >::type *dummy = 0)
{
    SUZERAIN_UNUSED(dummy);
...
template< typename FPT, typename Scale >
static void load223(
      typename suzerain::InterleavedState<std::complex<FPT> > &state,
      const Scale scale,
      typename boost::enable_if<boost::is_floating_point<FPT> >::type *dummy = 0)
{
    SUZERAIN_UNUSED(dummy);
...
template< typename FPT, typename Scale >
static void verify223(
      const typename suzerain::InterleavedState<std::complex<FPT> > &state,
      const Scale scale,
      typename boost::enable_if<boost::is_floating_point<FPT> >::type *dummy = 0)
{
    SUZERAIN_UNUSED(dummy);
...
BOOST_AUTO_TEST_CASE_TEMPLATE( constructors, T, test_types )
{
    ...
    for (int i = 0; i < foo.variable_count; ++i)
        for (int j = 0; j < foo.vector_length; ++j)
            for (int k = 0; k < foo.vector_count; ++k)
...
{
    suzerain::InterleavedState<T> foo(2, 2, 3), bar(2, 2, 3);
    load223(foo, 1);
...
{
    suzerain::InterleavedState<T> foo(2, 2, 2);
    suzerain::InterleavedState<T> bar(2, 2, 2);
    suzerain::InterleavedState<T> baz(1, 2, 2);
    suzerain::InterleavedState<T> qux(2, 1, 2);
    suzerain::InterleavedState<T> quux(2, 2, 1);
