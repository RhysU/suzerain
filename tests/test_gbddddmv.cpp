...
static void test_gbddddmv_s(const gbddddmv_tc_type& t)
{
    ...
    suzerain::unique_ptr<float[]> d2(new float[lend2]);
    suzerain::unique_ptr<float[]> d3(new float[lend3]);
...
static void test_gbddddmv_d(const gbddddmv_tc_type& t)
{
    ...
    suzerain::unique_ptr<double[]> d2(new double[lend2]);
    suzerain::unique_ptr<double[]> d3(new double[lend3]);
...
{
    const float close_enough = numeric_limits<float>::epsilon()*t.n*t.n*35000;
    const int lend0 = t.ldd0 * t.n;
    const int lend1 = t.ldd1 * t.n;
    const int lend2 = t.ldd2 * t.n;
    const int lend3 = t.ldd3 * t.n;
...
::boost::unit_test::test_suite* init_unit_test_suite( int, char* [] ) {
    ...
    // Register test_gbddddmv_scc and test_gbddddmv_ssc cases
    for (size_t i = 0; i < gcases; ++i) {
        ...
        { // Real-valued alpha, beta
            ...
            master_test_suite().add(make_test_case(&test_gbddddmv_ssc,
    ...
    // Register test_gbddddmv_dzz and test_gbddddmv_ddz cases
    for (size_t i = 0; i < gcases; ++i) {
        ...
        { // Real-valued alpha, beta
            ...
            master_test_suite().add(make_test_case(&test_gbddddmv_ddz,
