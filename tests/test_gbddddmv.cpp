...
static void test_gbddddmv_sc(const gbddddmzv_tc_type& t)
{
    ...
    // Compute observed result using our implementation
    BOOST_REQUIRE_EQUAL(0, suzerain_gbddddmv_sc(
            t.trans, t.n, t.kl, t.ku,
            alpha0, d0.get(), t.ldd0,
            alpha1, d1.get(), t.ldd1,
            alpha2, d2.get(), t.ldd2,
...
static void test_gbddddmv_dz(const gbddddmzv_tc_type& t)
{
    ...
    // Compute observed result using our implementation
    BOOST_REQUIRE_EQUAL(0, suzerain_gbddddmv_dz(
            t.trans, t.n, t.kl, t.ku,
            alpha0, d0.get(), t.ldd0,
            alpha1, d1.get(), t.ldd1,
            alpha2, d2.get(), t.ldd2,
...
init_unit_test_suite( int argc, char* argv[] )
{
    ...
    // Register test_gbddddmv_sc cases
    for (size_t i = 0; i < gcases; ++i) {
        ...
        { // Real-valued alpha, beta
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbddddmv_sc) << " real " << c;
            master_test_suite().add(make_test_case(
                    &test_gbddddmv_sc, name.str(), &c, &c + 1));
        }
        ...
        { // Imaginary-valued alpha, beta
            ...
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbddddmv_sc) << " imag " << c;
            master_test_suite().add(make_test_case(
                    &test_gbddddmv_sc, name.str(), &c, &c + 1));
        }
        ...
        { // Truly complex alpha, beta
            ...
            c.beta[0]   -= 1.5;
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbddddmv_sc) << " complex " << c;
            master_test_suite().add(make_test_case(
                    &test_gbddddmv_sc, name.str(), &c, &c + 1));
        }
    }

    // Register test_gbddddmv_dz cases
    for (size_t i = 0; i < gcases; ++i) {
        ...
        { // Real-valued alpha, beta
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbddddmv_dz) << " real " << c;
            master_test_suite().add(make_test_case(
                    &test_gbddddmv_dz, name.str(), &c, &c + 1));
        }
        ...
        { // Imaginary-valued alpha, beta
            ...
            std::swap(c.beta[0],   c.beta[1]);
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbddddmv_dz) << " imag " << c;
            master_test_suite().add(make_test_case(
                    &test_gbddddmv_dz, name.str(), &c, &c + 1));
        }
        ...
        { // Truly complex alpha, beta
            ...
            c.beta[0]   -= 1.5;
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbddddmv_dz) << " complex " << c;
            master_test_suite().add(make_test_case(
                    &test_gbddddmv_dz, name.str(), &c, &c + 1));
        }
    }
