...
// Test suzerain_gbidmv_?  against suzerain_blasext_*gbidmv_external
// Test suzerain_gbidmv_?? against suzerain_blasext_?gbidmzv_external
// Idea behind testing is that matching the BLAS is goodness

// For unary function-based test case registration
struct gbidmv_tc_type
{
    char trans;
    int n, kl, ku;
...
struct gbidmzv_tc_type
{
    ...
    gbidmzv_tc_type(const gbidmv_tc_type& o)
...
std::basic_ostream<charT,traits>& operator<<(
        std::basic_ostream<charT,traits> &os, const gbidmv_tc_type& t)
{
    os << "{trans="   << t.trans
       << ", n="      << t.n
       << ", kl="     << t.kl
...
template< typename charT, typename traits >
std::basic_ostream<charT,traits>& operator<<(
        std::basic_ostream<charT,traits> &os, const gbidmzv_tc_type& t)
{
    os << "{trans="   << t.trans
       << ", n="      << t.n
       << ", kl="     << t.kl
...
static void test_gbidmv_s(const gbidmv_tc_type& t)
{
    ...
    BOOST_REQUIRE_EQUAL(0, suzerain_gbidmv_s(
                  t.trans, t.n, t.kl, t.ku,
                  alpha0, alpha1, d1.get(),
                  a.get(), t.lda, x.get(), t.incx,
                  beta,           y.get(), t.incy));
...
static void test_gbidmv_d(const gbidmv_tc_type& t)
{
    ...
    suzerain_blasext_dgbidmv_external(
            t.trans, t.n, t.kl, t.ku,
            t.alpha0, t.alpha1, d1.get(),
            a.get(), t.lda, x.get(), t.incx,
            t.beta,         e.get(), t.incy);

    // Compute observed result using our implementation
    BOOST_REQUIRE_EQUAL(0, suzerain_gbidmv_d(
                t.trans, t.n, t.kl, t.ku,
                t.alpha0, t.alpha1, d1.get(),
                a.get(), t.lda, x.get(), t.incx,
                t.beta,         y.get(), t.incy));
...
static void test_gbidmv_sc(const gbidmzv_tc_type& t)
{
    ...
    // Compute observed result using our implementation
    BOOST_REQUIRE_EQUAL(0, suzerain_gbidmv_sc(
            t.trans, t.n, t.kl, t.ku,
            alpha0, alpha1, d1.get(),
            a.get(), t.lda, (const complex_float *) x.get(), t.incx,
...
static void test_gbidmv_dz(const gbidmzv_tc_type& t)
{
    ...
    // Compute observed result using our implementation
    BOOST_REQUIRE_EQUAL(0, suzerain_gbidmv_dz(
            t.trans, t.n, t.kl, t.ku,
            alpha0, alpha1, d1.get(),
            a.get(), t.lda, (const complex_double *) x.get(), t.incx,
...
init_unit_test_suite( int argc, char* argv[] )
{
    ...
    const gbidmv_tc_type gbidmv_tc[] = {
    ...
    const size_t gcases = sizeof(gbidmv_tc)/sizeof(gbidmv_tc[0]);

    // Register test_gbidmv_s cases
    for (size_t i = 0; i < gcases; ++i) {
        std::ostringstream name;
        name << BOOST_TEST_STRINGIZE(test_gbidmv_s) << ' ' << gbidmv_tc[i];
        master_test_suite().add(make_test_case(
                &test_gbidmv_s, name.str(), gbidmv_tc + i, gbidmv_tc + i + 1));
    }

    // Register test_gbidmv_d cases
    for (size_t i = 0; i < gcases; ++i) {
        std::ostringstream name;
        name << BOOST_TEST_STRINGIZE(test_gbidmv_d) << ' ' << gbidmv_tc[i];
        master_test_suite().add(make_test_case(
                &test_gbidmv_d, name.str(), gbidmv_tc + i, gbidmv_tc + i + 1));
    }

    // Register test_gbidmv_sc cases
    for (size_t i = 0; i < gcases; ++i) {

        gbidmzv_tc_type c(gbidmv_tc[i]);

        { // Real-valued alpha, beta
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbidmv_sc) << " real " << c;
            master_test_suite().add(make_test_case(
                    &test_gbidmv_sc, name.str(), &c, &c + 1));
        }
        ...
        { // Imaginary-valued alpha, beta
            ...
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbidmv_sc) << " imag " << c;
            master_test_suite().add(make_test_case(
                    &test_gbidmv_sc, name.str(), &c, &c + 1));
        }
        ...
        { // Truly complex alpha, beta
            ...
            c.beta[0]   -= 1.5;
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbidmv_sc) << " complex " << c;
            master_test_suite().add(make_test_case(
                    &test_gbidmv_sc, name.str(), &c, &c + 1));
        }
    }

    // Register test_gbidmv_dz cases
    for (size_t i = 0; i < gcases; ++i) {
        ...
        { // Real-valued alpha, beta
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbidmv_dz) << " real " << c;
            master_test_suite().add(make_test_case(
                    &test_gbidmv_dz, name.str(), &c, &c + 1));
        }
        ...
        { // Imaginary-valued alpha, beta
            ...
            std::swap(c.beta[0],   c.beta[1]);
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbidmv_dz) << " imag " << c;
            master_test_suite().add(make_test_case(
                    &test_gbidmv_dz, name.str(), &c, &c + 1));
        }
        ...
        { // Truly complex alpha, beta
            ...
            c.beta[0]   -= 1.5;
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbidmv_dz) << " complex " << c;
            master_test_suite().add(make_test_case(
                    &test_gbidmv_dz, name.str(), &c, &c + 1));
        }
    }
    ...
    for (size_t k = 0; k < max_fixed_bandwidth; ++k) {
        ...
        for (size_t i = 0; i < sizeof(fixed_tc)/sizeof(fixed_tc[0]); ++i) {
            ...
            gbidmv_tc_type r(fixed_tc[i]);
            r.kl   = r.ku = k;
            r.lda += (r.kl + r.ku + 1);

            { // Register test_gbidmv_s case
                std::ostringstream name;
                name << BOOST_TEST_STRINGIZE(test_gbidmv_s) << ' ' << gbidmv_tc[i];
                master_test_suite().add(make_test_case(
                        &test_gbidmv_s, name.str(), &r, &r + 1));
            }

            { // Register test_gbidmv_d case
                std::ostringstream name;
                name << BOOST_TEST_STRINGIZE(test_gbidmv_d) << ' ' << gbidmv_tc[i];
                master_test_suite().add(make_test_case(
                        &test_gbidmv_d, name.str(), &r, &r + 1));
            }
