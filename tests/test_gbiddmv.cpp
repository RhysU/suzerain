...
// Test suzerain_gbiddmv_?  against suzerain_blasext_*gbiddmv_external
// Test suzerain_gbiddmv_?? against suzerain_blasext_?gbiddmzv_external
// Idea behind testing is that matching the BLAS is goodness

// For unary function-based test case registration
struct gbiddmv_tc_type
{
    char trans;
    int n, kl, ku;
...
struct gbiddmzv_tc_type
{
    ...
    gbiddmzv_tc_type(const gbiddmv_tc_type& o)
...
std::basic_ostream<charT,traits>& operator<<(
        std::basic_ostream<charT,traits> &os, const gbiddmv_tc_type& t)
{
    os << "{trans="   << t.trans
       << ", n="      << t.n
       << ", kl="     << t.kl
...
template< typename charT, typename traits >
std::basic_ostream<charT,traits>& operator<<(
        std::basic_ostream<charT,traits> &os, const gbiddmzv_tc_type& t)
{
    os << "{trans="   << t.trans
       << ", n="      << t.n
       << ", kl="     << t.kl
...
static void test_gbiddmv_s(const gbiddmv_tc_type& t)
{
    ...
    suzerain_blasext_sgbiddmv_external(
            ...
            alpha0, alpha1, d1.get(), alpha2, d2.get(),
            a.get(), t.lda, x.get(), t.incx,
            beta,           e.get(), t.incy);

    // Compute observed result using our implementation
    BOOST_REQUIRE_EQUAL(0, suzerain_gbiddmv_s(
                  t.trans, t.n, t.kl, t.ku,
                  alpha0, alpha1, d1.get(), alpha2, d2.get(),
                  a.get(), t.lda, x.get(), t.incx,
                  beta,           y.get(), t.incy));
...
static void test_gbiddmv_d(const gbiddmv_tc_type& t)
{
    ...
    suzerain_blasext_dgbiddmv_external(
            t.trans, t.n, t.kl, t.ku,
            t.alpha0, t.alpha1, d1.get(), t.alpha2, d2.get(),
            a.get(), t.lda, x.get(), t.incx,
            t.beta,         e.get(), t.incy);

    // Compute observed result using our implementation
    BOOST_REQUIRE_EQUAL(0, suzerain_gbiddmv_d(
                t.trans, t.n, t.kl, t.ku,
                t.alpha0, t.alpha1, d1.get(), t.alpha2, d2.get(),
                a.get(), t.lda, x.get(), t.incx,
                t.beta,         y.get(), t.incy));
...
static void test_gbiddmv_sc(const gbiddmzv_tc_type& t)
{
    ...
    // Compute observed result using our implementation
    BOOST_REQUIRE_EQUAL(0, suzerain_gbiddmv_sc(
            t.trans, t.n, t.kl, t.ku,
            alpha0, alpha1, d1.get(), alpha2, d2.get(),
            a.get(), t.lda, (const float(*)[2]) x.get(), t.incx,
...
static void test_gbiddmv_dz(const gbiddmzv_tc_type& t)
{
    ...
    // Compute observed result using our implementation
    BOOST_REQUIRE_EQUAL(0, suzerain_gbiddmv_dz(
            t.trans, t.n, t.kl, t.ku,
            t.alpha0, t.alpha1, d1.get(), t.alpha2, d2.get(),
            a.get(), t.lda, (const double(*)[2]) x.get(), t.incx,
...
init_unit_test_suite( int argc, char* argv[] )
{
    ...
    const gbiddmv_tc_type gbiddmv_tc[] = {
    ...
    const size_t gcases = sizeof(gbiddmv_tc)/sizeof(gbiddmv_tc[0]);

    // Register test_gbiddmv_s cases
    for (size_t i = 0; i < gcases; ++i) {
        std::ostringstream name;
        name << BOOST_TEST_STRINGIZE(test_gbiddmv_s) << ' ' << gbiddmv_tc[i];
        master_test_suite().add(make_test_case(
                &test_gbiddmv_s, name.str(), gbiddmv_tc + i, gbiddmv_tc + i + 1));
    }

    // Register test_gbiddmv_d cases
    for (size_t i = 0; i < gcases; ++i) {
        std::ostringstream name;
        name << BOOST_TEST_STRINGIZE(test_gbiddmv_d) << ' ' << gbiddmv_tc[i];
        master_test_suite().add(make_test_case(
                &test_gbiddmv_d, name.str(), gbiddmv_tc + i, gbiddmv_tc + i + 1));
    }

    // Register test_gbiddmv_sc cases
    for (size_t i = 0; i < gcases; ++i) {

        gbiddmzv_tc_type c(gbiddmv_tc[i]);

        { // Real-valued alpha, beta
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbiddmv_sc) << " real " << c;
            master_test_suite().add(make_test_case(
                    &test_gbiddmv_sc, name.str(), &c, &c + 1));
        }
        ...
        { // Imaginary-valued alpha, beta
            ...
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbiddmv_sc) << " imag " << c;
            master_test_suite().add(make_test_case(
                    &test_gbiddmv_sc, name.str(), &c, &c + 1));
        }
        ...
        { // Truly complex alpha, beta
            ...
            c.beta[0]   -= 1.5;
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbiddmv_sc) << " complex " << c;
            master_test_suite().add(make_test_case(
                    &test_gbiddmv_sc, name.str(), &c, &c + 1));
        }
    }

    // Register test_gbiddmv_dz cases
    for (size_t i = 0; i < gcases; ++i) {
        ...
        { // Real-valued alpha, beta
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbiddmv_dz) << " real " << c;
            master_test_suite().add(make_test_case(
                    &test_gbiddmv_dz, name.str(), &c, &c + 1));
        }
        ...
        { // Imaginary-valued alpha, beta
            ...
            std::swap(c.beta[0],   c.beta[1]);
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbiddmv_dz) << " imag " << c;
            master_test_suite().add(make_test_case(
                    &test_gbiddmv_dz, name.str(), &c, &c + 1));
        }
        ...
        { // Truly complex alpha, beta
            ...
            c.beta[0]   -= 1.5;
            std::ostringstream name;
            name << BOOST_TEST_STRINGIZE(test_gbiddmv_dz) << " complex " << c;
            master_test_suite().add(make_test_case(
                    &test_gbiddmv_dz, name.str(), &c, &c + 1));
        }
    }
    ...
    const gbiddmv_tc_type fixed_tc[] = {
    ...
    for (size_t k = 0; k < max_fixed_bandwidth; ++k) {
        ...
        for (size_t i = 0; i < sizeof(fixed_tc)/sizeof(fixed_tc[0]); ++i) {
            ...
            gbiddmv_tc_type r(fixed_tc[i]);
            r.kl   = r.ku = k;
            r.lda += (r.kl + r.ku + 1);

            { // Register test_gbiddmv_s case
                std::ostringstream name;
                name << BOOST_TEST_STRINGIZE(test_gbiddmv_s) << ' ' << gbiddmv_tc[i];
                master_test_suite().add(make_test_case(
                        &test_gbiddmv_s, name.str(), &r, &r + 1));
            }

            { // Register test_gbiddmv_d case
                std::ostringstream name;
                name << BOOST_TEST_STRINGIZE(test_gbiddmv_d) << ' ' << gbiddmv_tc[i];
                master_test_suite().add(make_test_case(
                        &test_gbiddmv_d, name.str(), &r, &r + 1));
            }

            { // Register test_gbiddmv_sc cases
                gbiddmzv_tc_type c(r);
