...
\begin{align}
  ...
  \partial_{t} e      &= R_{e}\!\left(\rho,\vec{m},e\right)
                       + \mbox{Ma}^{2} \vec{f}_{m}(t) \cdot \vec{m}
\end{align}
\end{subequations}
...
form of the body acceleration $\vec{f}_{m}$ necessary to drive an isothermal
...
\begin{enumerate}
  ...
  \item Apply $\mbox{Ma}^{2}\vec{f}_{m}\cdot\vec{m} = \mbox{Ma}^{2} f_{m_x}
