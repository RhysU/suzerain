...
\fancypagestyle{plain}{
    ...
    \lhead{
        $L^2$ norm within Suzerain's discretization
    }
    \rhead{
...
holds, as it does for $\left|\left| u \right|\right|^{2}_{L^{2}_{xyz}}$,
$\left|\left| u \right|\right|^{2}_{L^{2}_{xz}} \!(y_j)$, and $\left|\left| u
\right|\right|^{2}_{L^{2}_{xz}} \!(y_j)$, the summands further simplify to
yield
}
  \sum_{m=-\frac{N_x}{2}}^{\frac{N_x}{2}-1}
  \sum_{n=-\frac{N_z}{2}}^{\frac{N_z}{2}-1}
