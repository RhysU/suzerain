...
\begin{align}
  \frac{\partial}{\partial{}t} u &= Lu + N\!\left(u\right)
\end{align}
on the spatial domain $\left[-\frac{L_x}{2},\frac{L_x}{2}\right] \times{}
[0,L_y] \times{} \left[-\frac{L_z}{2},\frac{L_z}{2}\right]$.  The operators $L$
and $N$ are linear and nonlinear, respectively.  To begin spatially
discretizing the system, we introduce its finite dimensional analog
\begin{align}
  \frac{\partial}{\partial{}t} u^h &= Lu^h + N\!\left(u^h\right) + R^h
...
With knowledge of the nonlinear operator $N$, such quadrature error can be
mitigated via an appropriate dealiasing technique (see,
...
discrete Fourier transforms and the requirements of the $L$ and $N$ operators.
...
\begin{align}
  ...
  &\approx
  L\left(\hat{u}_{y_{l'},m,n}(t)\right)
  +
...
having operators $L$ and $N$ operate on B-spline coefficients rather than
collocation point values.  For a discussion on moving operators between these
two representations, see \textsection{}5.5 in~\cite{Boyd2001}.
