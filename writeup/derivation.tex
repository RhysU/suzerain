...
&=
  ...
  \\
&=
  \sum_{l}\sum_{k_m}\sum_{k_n}
  \hat{u}_{l,m,n}(t)B_l(y)e^{\ii k_m x}e^{\ii k_n z}
  \label{eq:u_h_expansion}
\end{align}
where $k_m = 2\pi{}m/L_x$, $k_n = 2\pi{}n/L_z$, and $B_l(y)$ are a B-spline
basis for some order and knot selection.
...
``functions'' like $\delta(y-y_i) e^{\ii k_m' x}e^{\ii k_n' z}$ for fixed
collocation points $y_i$.  We catalog three relevant results
\begin{align}
   \int_0^{L_y} \varphi(y) \, \delta(y-y_i) \,d\!y
&= \varphi(y_i),
&
   \int_0^{L_x} e^{\ii k_m x} e^{-\ii k_m' x} \,d\!x
&= L_x \delta_{k_m k_m'}, \text{ and}
&
   \int_0^{L_z} e^{\ii k_n z} e^{-\ii k_n' z} \,d\!z
&= L_z \delta_{k_n k_n'}
\end{align}
...
\begin{align}
 ...
 R^h\!\left(x,y,z\right) \delta(y-y_i) e^{-\ii k_m' x}e^{-\ii k_n' z}
 \,d\!z \,d\!x \,d\!y
 &=
 \int_0^{L_x}\!\!\int_0^{L_z}
 R^h\!\left(x,y_i,z\right) e^{-\ii k_m' x}e^{-\ii k_n' z}
 \,d\!z \,d\!x
 =
 0
\end{align}
holds for all $y_i$, $k_m'$, and $k_n'$.  Inserting \eqref{eq:u_h_expansion}
...
\begin{align}
 ...
 &\partial_t
  ...
  \right)
  \left(
    \delta(y-y_i) e^{-\ii k_m' x}e^{-\ii k_n' z}
  \right)
  \, dz \, dx \, dy
\\
  &=
...
\\
  ...
  \, dz \, dx
\intertext{
  Reequating the two sides we have
}
  L_x L_z
  \sum_{l} B_l(y_i)
  \partial_t \hat{u}_{l,m,n}(t)
...
Finally, approximating the two integrals by discrete sums and moving the
resulting constants $N_x$ and $N_z$ to the left hand side leaves us with
\begin{align}
  L_x L_z N_x N_z
  \sum_{l} B_l(y_i)
  \partial_t \hat{u}_{l,m,n}(t)
...
\end{align}
Given knowledge of the nonlinear operator $N$, the introduced quadrature error
can be mitigated via an appropriate dealiasing technique (see,
e.g.,~\cite{Canuto2006}).  We are left with $N_x\times{}N_z$ time-dependent
systems containing $N_y$ equations coupled in the $x$ and $z$ directions only
through Fourier transform-like operations and the requirements of the nonlinear
operator $N$.
...
\end{enumerate}
Note that the left hand side contains a mass matrix arising from the B-spline
basis, collocation point choices, and constants $L_x$, $L_z$, $N_x$, and $N_z$.
Rolling $N_x$ and $N_z$ into the mass matrix allows us to hide the cost of
normalizing the forward fast Fourier transform.  We retain the mass matrix on
the same side as the time derivative in anticipation of the time discretization
scheme.
