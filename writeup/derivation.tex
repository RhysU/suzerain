...
fixed by $T_{0}$.  Note that these reference choices imply $a^{*}=\sqrt{T^{*}}$.
...
$\mathscr{L}$ and $\mathscr{N}$ are linear and nonlinear, respectively.  To
begin spatially discretizing the system, we introduce its finite dimensional
analog
\begin{align}
...
Finally, approximating the two integrals by discrete sums and multiplying
by $N_x$ and $N_z$ leaves us with
\begin{align}
  L_x L_z N_x N_z
  &\sum_{l} B_l\!\left(y_{l'}\right)
  \frac{\partial}{\partial{}t} \hat{u}_{l m n}(t)
...
time-independent mass matrix arising from the B-spline basis, collocation point
choices, and constants $L_x$, $L_z$, $N_x$, and $N_z$.  Rolling $N_x$ and $N_z$
into the mass matrix allows us to hide the cost of normalizing the forward fast
Fourier transform.  We retain the mass matrix on the same side as the time
derivative in anticipation of the time discretization scheme.
...
timestepper from appendix A of~\cite{spalart_lowstoragerk}.  This timestepper
advances the system
\begin{align}
 u_t &= Lu+N(u)
\end{align}
...
preceeding section's $\mathscr{L}$ and $\mathscr{N}$ (see
\textsection\ref{sec:combineddiscretization} for the exact relationships).
Both operators must be time-independent.  The SMR scheme advances according to
...
We wish to advance problems like $Mu_{t}=\tilde{L}u+\tilde{N}(u)$ where
$\tilde{L}$ and $\tilde{N}$ take state values to some non-state representation
which can be converted back to state by the linear ``mass matrix'' operator
$M$.  Then by $L=M^{-1}\tilde{L}$ and $N=M^{-1}\tilde{N}$ the general substep
equation gives
\begin{align}
  \left(I - \Delta{}t\beta_{i}M^{-1}\tilde{L}\right) u^{i+1}
  &=
  \left(I + \Delta{}t\alpha_{i}M^{-1}\tilde{L}\right) u^{i}
  + \Delta{}t\gamma_{i}M^{-1}\tilde{N}\left(u^{i}\right)
  + \Delta{}t\zeta_{i-1}M^{-1}\tilde{N}\left(u^{i-1}\right)
  .
\end{align}
...
\begin{align}
  ...
  + \Delta{}t\gamma_{i}\tilde{N}\left(u^{i}\right)
  + \Delta{}t\zeta_{i-1}\tilde{N}\left(u^{i-1}\right)
  \label{eq:generaloperatormasssubstep}
\end{align}
which is just \eqref{eq:generaloperatorsubstep} with $M$ replacing $I$,
$\tilde{L}$ replacing $L$, and $\tilde{N}$ replacing $N$.  We now drop the
tildes on $L$ and $N$ with the understanding that they are implied whenever
$M\neq{}I$.
...
\begin{algorithmic}
  ...
  \STATE $b\leftarrow{}b + \left(M+\Delta{}t\alpha_{i}L\right)a$
...
\begin{algorithmic}
  ...
  \STATE $b\leftarrow{}b+\left(M+\Delta{}t\alpha_{2}L\right)a$
...
\eqref{eq:spatial_discretization_gridpoints} fixes them instead as
\begin{subequations}
\begin{align}
   M u\bigr|_{m n}
&= \hat{u}_{y_{l'} m n}
...
\end{align}
\end{subequations}
In \eqref{eq:spatial_discretization} the operators $\tilde{L}$ and $\tilde{N}$
take B-spline coefficients as input and return collocation point values.  In
\eqref{eq:spatial_discretization_gridpoints} these operators both take and
return collocation point values.  We choose to work with
\eqref{eq:spatial_discretization} because it provides computational advantages.
For a discussion on moving operators between these two representations, see
\textsection{}5.5 in~\cite{Boyd2001}.
