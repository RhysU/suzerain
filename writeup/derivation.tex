...
matrix in the time derivative term.  We choose to work with
\eqref{eq:spatial_discretization} because there are computational advantages to
having operators $\mathscr{L}$ and $\mathscr{N}$ operate on B-spline
coefficients rather than collocation point values.  For a discussion on moving
operators between these two representations, see \textsection{}5.5
in~\cite{Boyd2001}.
...
preceeding sections' $\mathscr{L}$ and $\mathscr{N}$.  Both operators must be
time-independent.  The SMR scheme advances according to the following
equations:
\begin{subequations}
\begin{align}
  u'
