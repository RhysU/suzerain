...
&=
  ...
  \hat{u}_{l,m,n}(t)
  B_l(y)
  e^{i \frac{2\pi{}m}{L_x}x}
  e^{i \frac{2\pi{}n}{L_z}z}
  \\
&=
  \sum_{l}\sum_{k_x}\sum_{k_z}
  \hat{u}_{l,m,n}(t)B_l(y)e^{i k_x x}e^{i k_z z}
...
``functions'' like $\delta(y-y_i) e^{i k_x' x}e^{i k_z' z}$ for fixed
collocation points $y_i$.  We catalog three relevant results
\begin{align}
   \int_0^{L_y} \delta(y-y_i) \, \varphi(y) \,d\!y
&= \varphi(y_i),
&
   \int_0^{L_x} e^{i k_x x} e^{-i k_x' x} \,d\!x
&= L_x \delta_{k_x k_x'}, \text{ and}
&
   \int_0^{L_z} e^{i k_z z} e^{-i k_z' z} \,d\!z
&= L_z \delta_{k_z k_z'}
\end{align}
where the inner product's conjugate operation is accounted for by introducing a
negative sign into the latter two exponentials.  We substitute
\eqref{eq:u_h_expansion} into \eqref{eq:discrete_system_with_residual}, force
our residual $R^h$ to be zero at each $y_i$, and test with our chosen test
functions.  Simplifying each side separately we obtain the following:
\begin{align}
 ...
 &\partial_t
  ...
  \right)
  \left(
    \delta(y-y_i) e^{-i k_x' x}e^{-i k_z' z}
  \right)
  \, dz \, dx \, dy
\\
  &=
