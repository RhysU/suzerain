...
We wish to advance problems like $Mu_{t}=\tilde{L}u+\tilde{N}(u)$ where $M$ is
a linear ``mass matrix'' operator.  Then by $L=M^{-1}\tilde{L}$ and
$N=M^{-1}\tilde{N}$ the general substep equation gives
\begin{align}
...
$\tilde{L}$ replacing $L$, and $\tilde{N}$ replacing $N$.  We now drop
the tildes on $L$ and $N$ with the understanding that they are implied
whenever $M\neq{}I$.
...
\end{align}
\end{subequations}
Notice that the scaling factors on both $\tilde{L}$ and $\tilde{N}$ differ
between \eqref{eq:spatial_discretization} and
\eqref{eq:spatial_discretization_gridpoints}.

We choose to work with \eqref{eq:spatial_discretization} because there are
computational advantages to having $\mathscr{L}$ and $\mathscr{N}$ operate on
B-spline coefficients rather than collocation point values.  For a discussion
on moving operators between these two representations, see \textsection{}5.5
in~\cite{Boyd2001}.
