...
\begin{align}
  \frac{\partial}{\partial{}t} u &= N\!\left(u\right)
\intertext{
  on the spatial domain $\left[-\frac{L_x}{2},\frac{L_x}{2}\right] \times{}
  [0,L_y] \times{} \left[-\frac{L_z}{2},\frac{L_z}{2}\right]$, we introduce
  its finite dimensional analog
}
  \frac{\partial}{\partial{}t} u^h &= N\!\left(u^h\right) + R^h
...
applying \eqref{eq:R_h_weighted_residual_zero}, and simplifying each side
...
\end{align}
where $x_{m'}=L_x m' / N_x$ and $z_{n'}=L_z n' / N_z$.  Given knowledge of the
nonlinear operator $N$, the introduced quadrature error can be mitigated via an
appropriate dealiasing technique (see, e.g.,~\cite{Canuto2006}).
...
requirements of the nonlinear operator $N$.

Calling $\hat{u}_{l,m,n}(t)$ the wave space coefficients of the solution at
some time $t$, the right hand side of \eqref{eq:spatial_discretization} may be
efficiently computed by
\begin{enumerate}
 \item performing $m\times{}n$ matrix-vector products
       $\sum_{l} B_l\!\left(y_{l'}\right) \hat{u}_{l,m,n}(t)$,
