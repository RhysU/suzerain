...
function [Ma R0 R uR rhoR pR] = radialflow_match(delta, gam, Ma_e, p_exi, T_e=0)
...
%! delta = 1; gam = 1.4087;
%! [Ma R0 R uR rhoR pR] = radialflow_match(delta, gam, 1.1906, -0.025439, 4.0040);
%! [r u rho p]          = radialflow(Ma, gam, R, R0, uR, rhoR, pR);         % R->R0
%! [R1 u1 rho1 p1]      = deal(r(end), u(end), rho(end), p(end));           % R0
%! [Ma_e p_exi T_e]     = radialflow_qoi(delta, gam, Ma, R1, u1, rho1, p1); % R0->R
%! assert([Ma_e p_exi T_e], [1.1906, -0.025439, 4.0040], -sqrt(eps));

%!test % Round trip: Subsonic nozzle with less hot edge
%! delta = 1; gam = 1.4088;
%! [Ma R0 R uR rhoR pR] = radialflow_match(delta, gam, 0.54927, -0.014755, 3.1541);
%! [r u rho p]          = radialflow(Ma, gam, R, R0, uR, rhoR, pR);         % R->R0
%! [R1 u1 rho1 p1]      = deal(r(end), u(end), rho(end), p(end));           % R0
%! [Ma_e p_exi T_e]     = radialflow_qoi(delta, gam, Ma, R1, u1, rho1, p1); % R0->R
...
%! delta = 0.5; gam = 1.4;
%! [Ma R0 R uR rhoR pR] = radialflow_match(delta, gam, 1.5, +0.02, 0.50);
%! [r u rho p]          = radialflow(Ma, gam, R, R0, uR, rhoR, pR);         % R->R0
%! [R1 u1 rho1 p1]      = deal(r(end), u(end), rho(end), p(end));           % R0
%! [Ma_e p_exi T_e]     = radialflow_qoi(delta, gam, Ma, R1, u1, rho1, p1); % R0->R
